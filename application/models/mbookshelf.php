...
class mbookshelf extends CI_Model
{
  ...
  function get_user($uid){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         $data=$this->db->select('id,name_first,name_middle,name_last,email,user_type')->where('id',$uid)->get('users')->row_array();
        //echo "<pre>";print_r($data);echo "</pre>";die;
        //echo $this->db->last_query();die;
        return $data;               
    ...
    public function addToBookShelvesAjax($bkself_id,$wid){
        $data   = array();
        
        $usd = $this->session->userdata('logged_user');
        
        $rs=$this->db->select('*')->where('user_id',$usd['id'])->where('Wid',$wid)->where('bid',$bkself_id)->get('bookshelf_works');
        //echo $this->db->last_query();die;
        if($rs->num_rows() == 0){
    ...
    function get_publishers(){
         ...
         /*$my_email = $this->db->select('id,name_first,name_middle,name_last,user_type,email')->where('id',$usd['id'])->get('users')->row_array();
         $email_domain = explode('@',$my_email['email']); */
         //echo '<pre/>';print_r($my_email);die;
        
         
         /*$data=$this->db->select('id,name_first,name_middle,name_last,user_type,email')->where('id !=',$usd['id'])->where('user_type','2')->like('email', $email_domain[1])->get('users')->result_array(); */
         
         $data=$this->db->select('id,name_first,name_middle,name_last,user_type,email')->where('id !=',$usd['id'])->where('user_type','2')->get('users')->result_array();
    ...
    public function BookShelves_share(){
        ...
        //$email22 = explode(',',$data['email']);
        if(!empty($data['email']) && count($pub_email) > 0)
        {
        foreach($data['email'] as $email)
        {
          $pub_email=$this->db->select('id,email,user_type')->where('email',$email)->where('user_type','2')->get('users')->row_array();
          if(count($pub_email) > 0)
        {
            
        $data['note'] = $this->input->post('note');
        $data['bself_id'] = $this->input->post('bself_id');
        
        $bookshelf_name=$this->db->select('id,name,is_status')->where('id',$data['bself_id'])->where('is_status','1')->get('bookshelfs')->row_array();
        
        $row=$this->db->select('id,name_first,name_middle,name_last')->where('email',$email)->where('user_type','2')->get('users')->row_array();
        
        if(count($row) > 0)
        {
          $mail_send_id = $row['id'];
            
          $fullname_to = 'Dear '.$row['name_first'].' '.$row['name_middle'].' '.$row['name_last'].',';
          $data22['user_id'] = $usd['id'];    
          $data22['share_user_id'] = $mail_send_id;
          $data22['bookshelf_id'] = $this->input->post('bself_id');
          $data22['created_date']    = date("Y-m-d h:i:s");
          $data22['share_with'] = '1';
          $data22['status'] = '1';
           
          $this->db->insert('bookshelf_share', $data22);
        }
        else
        {
          $mail_send_id = $email;
         ...
         //echo "sssssss";die;
          ...
          $row22=$this->db->select('id,name_first,name_middle,name_last')->where('id',$usd['id'])->get('users')->row_array();
          $fullname_from = $row22['name_first'].' '.$row22['name_middle'].' '.$row22['name_last'];
          
            $data33['to_user_id'] = $mail_send_id;
		    $data33['from_user_id'] = $usd['id'];
		    
		    $data33['subject'] = 'Shared Bookshelf from '.$fullname_from;
		    
		    $data33['body'] = '<table width="100%" border="0" cellpadding="8" cellspacing="0" style="color:#39302c; padding:10px 0 0px 0px;">
   
                               <tr>
                               <td width="27%" style="color: #000;"><p>'.$fullname_to.'</p><br/>
                            
                                <p>I am sharing one of my favorite bookshelves, '.$bookshelf_name['name'].', with you. Please <a href="'.base_url().'bookshelves">click here</a> to view the bookshelf  and let me know what you think.</p><br/>
                                
                                <p>Kind Regards,</p><br/>
                                
                                <p>'.$row22['name_first'].'</p>
           ...
           $this->send_mail_to_registered_user($email, $data['bself_id'], $fullname_to, $row22['name_first'], $bookshelf_name['name']); 
   ...
   function send_mail_to_registered_user($email, $bselfid, $fullname_to, $fullname_from, $bookshelfname){
        
        //$sub    = 'Inkubate Shared Bookshelf';
        $sub    = 'Shared bookshelf message';
        
        $str = '<table width="100%" border="0" cellpadding="8" cellspacing="0" style="color:#39302c; padding:10px 0 0px 0px;">
   
                               <tr>
                               <td width="27%" style="color: #000;"><p>'.$fullname_to.'</p>
                                <div class="clear"></div>
                                <p>I am sharing one of my favorite bookshelves, '.$bookshelfname.', with you. Please <a href="'.base_url().'bookshelves">click here</a> to view the bookshelf  and let me know what you think.</p><br/>
   ...
   function get_bookshelf(){
    ...
    function get_bookshelf_by_share($search="",$offset=null,$limit=null){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         if($search == "")
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.share_user_id', $usd['id'])->where('s.share_by', '0')->where('a.is_status !=','0')->order_by('a.id','desc')->limit($limit,$offset)->get()->result_array();
         ...
         else
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.share_user_id', $usd['id'])->where('s.share_by', '0')->where('a.is_status !=','0')->like('name', $search)->limit($limit,$offset)->get()->result_array();
            //echo $this->db->last_query();die;
         }
    ...
    function get_bookshelf_by_share_count($search="",$offset=null,$limit=null){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         if($search == "")
         {
         ...
         else
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.share_user_id', $usd['id'])->where('s.share_by', '0')->where('a.is_status !=','0')->like('name', $search)->get()->result_array();
            //echo $this->db->last_query();die;
         }
    ...
    function get_bookshelf_with_share_total($search="",$offset=null,$limit=null){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         if($search == "")
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.user_id', $usd['id'])->where('s.share_with', '1')->where('a.is_status !=','0')->order_by('s.id','desc')->get()->result_array();
         ...
         else
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.user_id', $usd['id'])->where('s.share_with', '1')->where('a.is_status !=','0')->like('name', $search)->get()->result_array();
            //echo $this->db->last_query();die;
         }
    ...
    function get_bookshelf_with_share($search="",$offset=null,$limit=null){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         if($search == "")
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.user_id', $usd['id'])->where('s.share_with', '1')->where('a.is_status !=','0')->order_by('s.id','desc')->group_by("a.id")->limit($limit,$offset)->get()->result_array();
         ...
         else
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.user_id', $usd['id'])->where('s.share_with', '1')->where('a.is_status !=','0')->like('name', $search)->group_by("a.id")->limit($limit,$offset)->get()->result_array();
            //echo $this->db->last_query();die;
         }
    ...
    function get_bookshelf_with_share_count($search="",$offset=null,$limit=null){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         if($search == "")
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.user_id', $usd['id'])->where('s.share_with', '1')->where('a.is_status !=','0')->order_by('s.id','desc')->group_by("a.id")->get()->result_array();
         ...
         else
         {
            ...
            $data  = $this->db->select('s.id as share_id,s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.user_id', $usd['id'])->where('s.share_with', '1')->where('a.is_status !=','0')->like('name', $search)->group_by("a.id")->get()->result_array();
            //echo $this->db->last_query();die;
         }
    ...
    function get_bookshelf_user_first($uid)
    {
      $data=array();
         $usd = $this->session->userdata('logged_user');
         $data=$this->db->select('id,name_first,name_middle,name_last,email')->where('id',$uid)->limit(1)->get('users')->row_array(); 
         return $data;  
    }
    ...
    function get_user_book_self_count($id){
         ...
         $data  = $this->db->select('bw.*')->from('bookshelf_works as bw')->join('works as w','w.id = bw.Wid')->where('bw.bid', $id)->get()->result_array();
        //echo "<pre>";print_r($data);echo "</pre>";die;
        //echo $this->db->last_query();die;
        return count($data);            
   ...
   function saveTitle($wid){
    ...
    function getSearchInfoBySearch($search)
    {
    	$usd = $this->session->userdata('logged_user');
    	$info = array();
        $sql_request = "`work_type_name`  LIKE '%". $search ."%'";
    	$data['types'] = $this->db->select('work_type_id')->where($sql_request, null, false)->get('work_types')->result_array();
        
        $sql_request1 = "`work_form_name`  LIKE '%". $search ."%'";
    	$data['format'] = $this->db->select('work_form_id')->where($sql_request1, null, false)->get('work_forms')->result_array();
        
        $sql_request2 = "`category_name`  LIKE '%". $search ."%'";
    	$data['genre'] = $this->db->select('id')->where($sql_request2, null, false)->where('is_show','1')->get('categories')->result_array();
       ...
       $usd = $this->session->userdata('logged_user');
    	...
    	else if($type == "multiple")
    	{
    		//print_r($search);echo $subtype." ".$searchCriteria;exit;
    		    ...
    		    if($subtype == "global")
    		    {
    		        $sql_request = "`tag_name`  LIKE '%". $searchCriteria ."%'";
    		    	$tags  = $this->db->select('tag_name,id')->from('tags')->where($sql_request, null, false)->get()->result_array();
    		   	...
    		   	$this->db->join('tags as wt','wt.wid = w.id','left'); 		   	
                ...
                $sql .= " OR concat(u.name_first,' ',u.name_middle,' ',u.name_last) LIKE '%$searchCriteria%'";
                $sql .= " OR concat(u.name_first,' ',u.name_middle) LIKE '%$searchCriteria%'";
                $sql .= " OR concat(u.name_first,' ',u.name_last) LIKE '%$searchCriteria%'"; 
                ...
                /*$sql = " MATCH(w.title) AGAINST ('".$searchCriteria."')";
	    		
                $sql .= " OR concat(u.name_first,' ',u.name_middle,' ',u.name_last) LIKE '%$searchCriteria%'";
                $sql .= " OR concat(u.name_first,' ',u.name_middle) LIKE '%$searchCriteria%'";
                $sql .= " OR concat(u.name_first,' ',u.name_last) LIKE '%$searchCriteria%'"; 
               
                $sql .= " OR MATCH(w.synopsis) AGAINST ('".$searchCriteria."')";
	    		$sql .= " OR MATCH(w.extract) AGAINST ('".$searchCriteria."')"; */
                
                
                $arr1 = str_split($searchCriteria);
                $arr2 = EXPLODE(" OR ",$searchCriteria);
                //print_r($arr2).'<br/>';
                //echo $arr1[1];
                ...
                $sql .= " ( w.title REGEXP '[[:<:]]".$searchCriteria."[[:>:]]'";
                $sql .= " OR concat(u.name_first,' ',u.name_middle,' ',u.name_last) LIKE '$searchCriteria'";
                $sql .= " OR concat(u.name_first,' ',u.name_middle) LIKE '$searchCriteria'";
                $sql .= " OR concat(u.name_first,' ',u.name_last) LIKE '$searchCriteria'";
                $sql .= " OR concat(u.name_middle,' ',u.name_last) LIKE '$searchCriteria'";
                
                $sql .= " OR u.name_first LIKE '$searchCriteria'"; 
                $sql .= " OR u.name_middle LIKE '$searchCriteria'";
                $sql .= " OR u.name_last LIKE '$searchCriteria'";
               
                $sql .= " OR w.synopsis REGEXP '[[:<:]]".$searchCriteria."[[:>:]]'";
	    		$sql .= " OR w.extract REGEXP '[[:<:]]".$searchCriteria."[[:>:]]' ) ";  
                ...
                $words = EXPLODE(" OR ",$searchCriteria);
                ...
                FOR($i=0;$i<count($words);$i++)
                   {
                    ...
                    $query_string .= " w.title REGEXP '[[:<:]]".$words[$i]."[[:>:]]'";
                    
                    $query_string .= " OR concat(u.name_first,' ',u.name_middle,' ',u.name_last) LIKE '".$words[$i]."'";
                    $query_string .= " OR concat(u.name_first,' ',u.name_middle) LIKE '".$words[$i]."'";
                    $query_string .= " OR concat(u.name_first,' ',u.name_last) LIKE '".$words[$i]."'";
                    $query_string .= " OR concat(u.name_middle,' ',u.name_last) LIKE '".$words[$i]."'";
                    
                    $query_string .= " OR u.name_first LIKE '".$words[$i]."'"; 
                    $query_string .= " OR u.name_middle LIKE '".$words[$i]."'";
                    $query_string .= " OR u.name_last LIKE '".$words[$i]."'";
                   
                    $query_string .= " OR w.synopsis REGEXP '[[:<:]]".$words[$i]."[[:>:]]'";
    	    		$query_string .= " OR w.extract REGEXP '[[:<:]]".$words[$i]."[[:>:]]' OR ";
                ...
                {
                 $words = EXPLODE(" ",$searchCriteria);
                
                $query_string = '';
                 
                FOR($i=0;$i<count($words);$i++)
                   {
                    /*$query_string .= " w.title IN '(".$words[$i].")'";
                    $query_string .= " AND u.name_first IN '(".$words[$i].")'";
                    $query_string .= " AND u.name_middle IN '(".$words[$i].")'";
                    $query_string .= " AND u.name_last IN '(".$words[$i].")'";
                    $query_string .= " AND w.synopsis IN '(".$words[$i].")'";
                    $query_string .= " AND w.extract IN '(".$words[$i].")'"; */
                    ...
                    $query_string .= " OR w.synopsis REGEXP '[[:<:]]".$words[$i]."[[:>:]]'";
    	    		$query_string .= " OR w.extract REGEXP '[[:<:]]".$words[$i]."[[:>:]]' ) AND ";
    ...
    function getSelectFilterInfo($type,$data)
    {
    	...
    	if($type == "genre")
    	{
    		$values = $this->db->select('id,category_name,\'genre\' as `type`',false)->where_in('id', $data)->where('is_show', '1')->get('categories')->result_array();
    	}
    	if($type == "multiple")
    	{
    	   if(isset($data['types']) && count($data['types']) > 0)
    		...
    		{
    			$genre = $this->db->select('id,category_name,\'genre\' as `subtype`,\'multiple\' as `type`',false)->where_in('id', $data['genre'])->where('is_show', '1')->get('categories')->result_array();
   ...
   // print_r($search);
    ...
    // echo $search[0];
         ...
         if($type == "format")
    	{
    	   if(!empty($search[0]))
  		    {
    		$rs  = $this->db->select('*')->where_in('work_form_id', $search)->order_by('id','desc')->limit($limit,$offset)->get('works');
            //echo $this->db->last_query();
    	...
    	else if($type == "types")
    	{
    		if(!empty($search[0]))
  		    {
    		$rs  = $this->db->select('*')->where_in('work_type_id', $search)->order_by('id','desc')->limit($limit,$offset)->get('works');
            }
            else
            {
             $this->db->select('w.*,c.category_name');
    	...
    	else if($type == "genre")
    	{
    	   if(!empty($search[0]))
  		    {
    		$rs=$this->db->select('w.*,c.category_name')->from('works w')->join('work_categories as wc','w.id = wc.Wid','inner')->join('categories as c','c.id = wc.cid','inner')->where_in('cid', $search)->group_by('w.id')->order_by('w.id','desc')->limit($limit,$offset)->get();
            }
            else
            {
             $this->db->select('w.*,c.category_name');
    	...
    	else if($type == "multiple")
    	{
                ...
                //$words_single = EXPLODE('"',$searchCriteria);
                $arr1 = str_split($searchCriteria);
                $arr2 = EXPLODE(" OR ",$searchCriteria);
                //print_r($arr2).'<br/>';
                //echo $arr1[0];
                ...
                {
                   $searchCriteria = str_replace('"','',$searchCriteria);
                   //echo $searchCriteria = str_replace("'",'',$searchCriteria);
               ...
               /* $words = EXPLODE(" ",$searchCriteria);
	    		...
	    		//else
	    		//{
	    			//if conditon for subtype global get value form textbox 2 if condition
	    		//}
	    		if(count($work_tags) > 0)
	    		{
	    			$tgs = implode(",",$work_tags)  ;	
    		    ...
    		    /*if($subtype == "global")
    		    {
	    		    ...
	    		    if(count($search['genre']) > 0)
	    		    {
	    		    	$this->db->or_where_in('cid', $search['genre']);f
	    		    }
    		    }
    		    else
    		    {
    	...
    	{
    		$rs  = $this->db->select('*')->order_by('id','desc')->limit($limit,$offset)->get('works');
            //echo $this->db->last_query();
    	}
        ...
        if($rs->num_rows() > 0 && ($type == "format" || $type == "types" || $type == "")){
            ...
            foreach($data as $each=>$value){
                ...
                //$row=$this->db->where('id',$value['user_id'])->where('status_id','1')->get('users')->row_array();
                $row=$this->db->where('id',$value['user_id'])->get('users')->row_array();
                if(!empty($value['user_id'])){
                    $user_photo = $this->mprofile->getUserPhotoById($value['user_id']);
                }
                //print_r($row);
                //echo $this->db->last_query();
                if(!empty($row['name_first']))
                ...
                if(!empty($user_photo))
                {
                $data[$each]['user_image'] = $user_photo['filename'];
                }
                else
                {
                 $data[$each]['user_image'] = '';   
                }
                ...
                $row44=$this->db->select('c.category_name,c.id')->where('wc.Wid',$value['id'])->from('work_categories as wc')->join('categories as c','c.id = wc.cid','inner')->get()->row_array();
  ...
  function getAllFormat()
    ...
    function getActiveCategories( )
    {
        ...
        foreach($data as $key=>$row) {
          $data_name[$key] = strtolower(trim($row['category_name']));
        } 
        array_multisort($data_name, SORT_ASC, $data);
        //echo "<pre>";print_r($data);echo "</pre>";die;
        return $data;
    } 
  ...
  function get_book_list($bid){
   ...
   function delete_bookshelf_new($id)
    ...
    function delete_bookshelf_share_with($id)
    {
        //$this->db->where('id',$id)->delete('bookshelf_share');
        //$this->db->where('bid',$id)->delete('bookshelf_works');
        $data['share_with'] = "0";
    	$this->db->where('id',$id)->update("bookshelf_share",$data);
    }
    
    function delete_bookshelf_share_by($id)
    {
        //$this->db->where('id',$id)->delete('bookshelf_share');
        //$this->db->where('bid',$id)->delete('bookshelf_works');
        $data['share_by'] = "1";
            ...
            if(!empty($saved_title_bookshelf))
                {
                    ...
                    {
                        $full_name_saved = $total_work_saved['name'].' '.$total_work_saved['name_middle'].' '.$total_work_saved['name_last'];
                ?>
                ...
                <div class="no_profile_pic">
                                	<div style="text-align: center; font-weight:600; line-height:15px; padding-top:25px; font-size: 10px; cursor: pointer;" title="<?=$total_work_saved['title']?>"><?=substr(strtoupper($total_work_saved['title']),0,13)?></div>
                                
                </div>
                
                <?php } ?>
                </div>
                <div class="item_right_section" style="width:98px;">
                <p>
                Author: <?//=$total_work_saved['name']?>
                
                <?php
                if(strlen($full_name_saved) > 6)
                    {
                        ?>
                     <a href="javascript:void(0);" class="tooltips">   
                    <?php    
                     echo substr($full_name_saved,0,6).'..';
                     ?>
    ...
    function search_book_title($val)
    {
        ...
        if(count($workdetails_total) > 0)
        {
           ...
           <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_new orange1234" id="table_bookshelf">
              ...
              <?php  if(file_exists("uploadImage/".$total_work['user_id']."/cover_image/medium/".$total_work['photo']) && $total_work['photo'] != '') {?>
