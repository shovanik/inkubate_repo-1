...
class mbookshelf extends CI_Model
{
  ...
  function total_saved_search(){
    ...
    public function BookShelves_share(){
        ...
        $row=$this->db->select('id')->where('email',$this->input->post('email'))->get('users')->row_array();
   ...
   function get_bookshelf(){
    ...
    function get_bookshelf_with_share($search="",$offset=null,$limit=null){
         ...
         if($search == "")
         {
            ...
            $data  = $this->db->select('s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.share_user_id', $usd['id'])->where('a.is_status !=','0')->order_by('a.id','desc')->limit($limit,$offset)->get()->result_array();
         ...
         else
         {
            ...
            $data  = $this->db->select('s.user_id as uid,s.share_user_id as shuid,s.bookshelf_id as bid,s.created_date as date,a.*')->from('bookshelf_share as s')->join('bookshelfs as a','s.bookshelf_id = a.id','inner')->where('s.share_user_id', $usd['id'])->where('a.is_status !=','0')->like('name', $search)->limit($limit,$offset)->get()->result_array();
            //echo $this->db->last_query();die;
         }
