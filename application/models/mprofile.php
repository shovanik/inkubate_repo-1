 ...
 function total_publisher_details($id,$type_id,$form_id)
   ...
   function total_publisher_details_forpitch()
     ...
     function get_user_country($country_id)
    {
        $data=array();
         $usd = $this->session->userdata('logged_user');
         $data  = $this->db->select('*')->where('country_code',$country_id)->get('countries')->row_array();
       //echo $this->db->last_query();die;
        return $data;
    ...
    public function updateProfile(){
       ...
       //echo $_FILES['image']['name'].'hi';die;
        ...
        $prfl_wrt_bio  = $this->db->select('*')->where('user_id',$usd['id'])->get('profile_writer'); 
        
        if($prfl_wrt_bio->num_rows() > 0)
          { 
             
              $this->db->where('user_id',$usd['id'])->update('profile_writer',$data22);  
        ...
        //echo $this->db->last_query().'hello';die;
        ...
        //echo $pid;die;
            ...
            $data58['email']  = $this->input->post('personal_email');
           ...
           $pass = $this->input->post('password');
            if($pass != '')
            {
            $data58['password']  = md5($this->input->post('password'));
            }
        ...
        if($this->input->post('facebook') != '')
            ...
            $data58['title_publish']  = $this->input->post('title_publish');
            $this->db->where('id',$usd['id'])->update('users',$data58);
            //echo $this->db->last_query().'hi';die;
  ...
  function work_cat_select($id=null,$catid=null){
   ...
   $data=$this->db->select('*')->where('user_id',$usd['id'])->get('profile_writer')->row_array();
    ...
    function get_user_picture(){
         ...
         $usd = $this->session->userdata('logged_user');
   //$data  = $this->db->select('s.*,a.*')->from('assets as s')->join('profile_writer as a','a.h_asset_id = s.id','inner')->where('s.user_id', $usd['id'])->get()->row_array();
         $data=$this->db->select('id,user_id,description,filename')->where('user_id',$usd['id'])->where('status_id','1')->get('assets')->row_array();
        //echo "<pre>";print_r($data);echo "</pre>";die;
        //echo $this->db->last_query();die;
        return $data;               
   ...
   $data=$this->db->select('*')->where('id',$usd['id'])->get('users')->row_array();
    ...
    function get_user_photo(){
         ...
         $usd = $this->session->userdata('logged_user');
   //$data  = $this->db->select('s.*,a.*')->from('assets as s')->join('profile_writer as a','a.h_asset_id = s.id','inner')->where('s.user_id', $usd['id'])->get()->row_array();
      $data22=$this->db->select('*')->where('user_id',$usd['id'])->where('description','profile')->order_by('id','desc')->limit(1)->get('assets')->row_array();
        //echo "<pre>";print_r($data);echo "</pre>";die;
        //echo $this->db->last_query();die;
        return $data22;               
 ...
 function delete_msg($id)
  ...
  function check_work($id){
   ...
   function check_social($id){
    ...
    {
      $data=array();
      $rs  = $this->db->select('*')->where('country_code !=', "US")->order_by('country_name','ASC')->get('countries');
      if($rs->num_rows() > 0){
        $data   = $rs->result_array();
      }
      return $data;        
