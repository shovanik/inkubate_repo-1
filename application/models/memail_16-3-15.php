...
class memail extends CI_Model
{
   ...
   //$rs  = $this->db->select('s.*,a.*')->from('work_pitchits as s')->join('pitchit_view as a','s.cid = a.cid','inner')->order_by('a.id','desc')->group_by('s.user_id')->limit(4)->get();
      ...
      function get_publisher_totalview_pitchit(){
         $data=array();
         $usd = $this->session->userdata('logged_user');
   
   $rs  = $this->db->select('s.id as pitid,s.user_id as pituser,s.wid as pitwid,s.pitchit_id as pitchitid,s.date as pitdate,a.pit_id,a.user_id,a.pitchit')->from('pitchit_view as s')->join('work_pitchits as a','s.pitchit_id = a.pit_id','inner')->where('a.user_id', $usd['id'])->order_by('s.id','desc')->limit(6)->get();
  ...
  function get_user_work_details($offset=null,$limit=null){
   ...
   $rs  = $this->db->select('s.*,a.*')->from('work_pitchits as s')->join('works as a','s.wid = a.id','inner')->where('s.user_id', $usd['id'])->where('s.is_pitchit', '1')->where('s.is_drafted', '0')->order_by('s.created_date','desc')->limit(6)->get();
   ...
   function get_user_pitchit_saved_details(){
         $data=array();
         $usd = $this->session->userdata('logged_user');
  
   $rs  = $this->db->select('s.*,a.*')->from('work_pitchits as s')->join('works as a','s.wid = a.id','inner')->where('s.user_id', $usd['id'])->where('s.is_pitchit', '1')->where('s.is_drafted', '1')->order_by('s.created_date','desc')->limit(6)->get();
         //$rs=$this->db->select('*')->where('user_id',$usd['id'])->order_by('create_date','desc')->limit(4)->get('works');
      //echo $this->db->last_query();die;
   ...
   return $rs;
    ...
    function get_user_pitchit_details_publisher(){
         $data=array();
         $usd = $this->session->userdata('logged_user');
   ...
   $rs  = $this->db->select('s.*,a.id,a.title')->from('work_pitchits as s')->join('works as a','s.wid = a.id','inner')->where('s.is_drafted','0')->order_by('s.created_date','desc')->limit(5)->get();
         //$rs=$this->db->select('*')->where('user_id',$usd['id'])->order_by('create_date','desc')->limit(4)->get('works');
      //echo $this->db->last_query();die;
   ...
   //$rs  = $this->db->select('s.*,a.*')->from('work_pitchits as s')->join('works as a','s.wid = a.id','inner')->where('s.user_id', $usd['id'])->order_by('s.created_date','desc')->limit(3)->get();
         $rs=$this->db->select('id')->where('wid',$wid)->get('pitchit_view');
      //echo $this->db->last_query();die;
  ...
  function get_user_pitchit_view_user($wid){
   ...
   $rs  = $this->db->select('s.*,a.name_first,a.name_middle,a.name_last')->from('pitchit_view as s')->join('users as a','s.user_id = a.id','inner')->where('s.pitchit_id', $wid)->order_by('id','desc')->get()->row_array();
    ...
    function get_user_pitchit_msg_total_reply($pit_id){
         ...
         $rs  = $this->db->select('count(*) as count')->from('messages')->where('is_pitchited', '1')->where('is_viewed', '0')->where('to_user_id',$usd['id'])->where('pitchit_id',$pit_id)->get()->row_array();
  ...
  function update_pitchit_view_19($pitid)
    ...
    function pitchitCount(){
         $data=array();
         $usd = $this->session->userdata('logged_user');
         $rs  = $this->db->select('count(*) as count')->where('is_deleted !=', '1')->where('is_drafted', '0')->where('is_moved', '0')->where('is_viewed', '0')->where('to_user_id', $usd['id'])->where('is_pitchited', '1')->order_by('created','desc')->get('messages');
        //echo $this->db->last_query();die;
        //$this->db->limit(1,0);
        if($rs->num_rows() > 0){
            $data   = $rs->result_array();
   ...
   $data  = $this->db->select('s.*, s.created msg_created,a.*')->from('messages as s')->join('users as a','a.id = s.to_user_id','inner')->where('s.id', $id)->where('a.status_id','1')->get()->row_array();
    ...
    function replymailInfo($id){
         $data=array();
         $usd = $this->session->userdata('logged_user');
   $rs  = $this->db->select('s.*, s.created msg_created, a.*')->from('messages as s')->join('users as a','a.id = s.to_user_id','inner')->where('s.to_reply_id', $id)->where('a.status_id','1')->get();
        //echo "<pre>";print_r($data);echo "</pre>";die;
        //echo $this->db->last_query();die;
        if($rs->num_rows() > 0){
            $data   = $rs->result_array();
...
function unmark_msg($msg_type,$id,$offset=0,$limit=15)
{
 ...
 function delete_msg($msg_type, $id,$offset=0,$limit=15)
  ...
  function get_author_view_details($offset=null,$limit=null){
       ...
       $rs  = $this->db->select('*')->from('view_title')->where('user_id !=', $usd['id'])->order_by('created_date','desc')->limit($limit,$offset)->get();
    ...
    function get_author_view_details_cnt(){
       ...
       $data  = $this->db->select('count(*) as count')->from('view_title')->where('user_id !=',$usd['id'])->get()->row_array();
