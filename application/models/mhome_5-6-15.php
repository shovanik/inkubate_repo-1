   ...
   function get_twit_user($twid)
    ...
    function get_google_user($twid)
    {
        $data=array();
         ...
         $data  = $this->db->select('count(*) as count,user_type')->where('social_id', $twid)->get('users')->row_array();
    ...
    public function social_register()
    {
        ...
        if($chkRs['count'] == 0){
            ...
            $data22['user_type'] = $this->input->post('type');
    ...
    function forget_password(){
        ...
        $fullname = $get_user['name_first'].' '.$get_user['name_middle'].' '.$get_user['name_last'];
...
<p>If you want to change the password, please login to Inkubate with this password.</p>
...
<p>Your request for an invitation to Inkubate has been approved. Please click <a href="http://billbahadur.com/demo/inkubate/home/success_sign/'.$unqid.'">http://billbahadur.com/demo/inkubate/home/success_sign/'.$unqid.'</a> to finish the registration process. *If this link does not work, copy and paste the URL into your browser.</p>
