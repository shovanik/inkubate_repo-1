...
/*<p>Please <a href="http://billbahadur.com/demo/inkubate/bookshelves/sharebooklist/'.$bselfid.'">click here</a> to view</p>*/
  ...
  function get_main_org_pitchit(){
   ...
   //$data  = $this->db->select('s.*, s.created msg_created,a.*')->from('messages as s')->join('users as a','a.id = s.to_user_id','inner')->where('s.id', $id)->where('a.status_id','1')->get()->row_array();
        ...
        $data  = $this->db->select('s.*,s.created msg_created,a.id as uid,a.name_first,a.name_middle,a.name_last,a.gender,a.user_type,ast.description,ast.filename')->from('messages as s')->join('users as a','a.id = s.to_user_id','left')->join('assets as ast','ast.user_id = s.to_user_id','left')->where('s.id', $id)->where('a.status_id','1')->where('ast.description', 'profile')->get()->row_array();
