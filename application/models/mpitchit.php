...
class mpitchit extends CI_Model
{
    ...
    function addpitchit()
    {
        ...
        $data['allow_hour'] = $this->input->post('hour');
      ...
      //die;
       ...
       //echo '<pre/>';print_r($data);exit;
        foreach($data as $each=>$value){
                ...
                $row25=$this->db->select('id,name_first,name_middle,name_last,user_type')->where('id',$value['pvuid'])->get('users')->row_array();
                ...
                if(!empty($row25['user_type']))
                {
                $data[$each]['user_type'] = $row25['user_type'];
