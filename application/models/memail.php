...
class memail extends CI_Model {
		...
		if ($type == 'send') {
			...
			if (count($result) > 0) {
				foreach ($result as $key => $row) {
					...
					//print_r ($str1);die;
					$from_user_name = $this->db->select('id,name_first,name_middle,name_last,email,status_id')->where('id', $usd['id'])->where('status_id', '1')->get('users')->row_array();

					$user_total_name = $from_user_name['name_first'] . ' ' . $from_user_name['name_middle'] . ' ' . $from_user_name['name_last'];
                    $user_total_name_email = $from_user_name['email'];
					//$to_user_total_name = $row['name_first'] . ' ' . $row['name_middle'] . ' ' . $row['name_last'];
                    $to_user_total_name = $row['name_first'];

					$this->send_mail_to_registered_user_forCompose($row['email'], $user_total_name, $to_user_total_name, $data['body'], $user_total_name_email);
				}
			}
	...
	function send_mail_to_registered_user_forCompose($email, $user, $touser, $message, $fromemail) {

		$sub = 'A Message from ' . $user;
  ...
  <tr style="background-color: #fff;">
    <td width="27%" style="color: #000;"><p>Dear ' . $touser . ',</p>

    <p>You have a message from ' . $user . ', please <a href="http://billbahadur.com/demo/inkubate/home/login/msg">click here</a> to respond to '. $user . '.</p>
    
    <p>Kind Regards,</p>
    <p>The Inkubate Team</p>
    </td>
  </tr>
...
/*<p>Please <a href="http://billbahadur.com/demo/inkubate/bookshelves/sharebooklist/'.$bselfid.'">click here</a> to view</p>*/
        ...
        $this->email->from($fromemail, $user);
        $this->email->to($email); 
	...
	function get_publisher_totalview_pitchit($offset = null, $limit = null) {
		...
		if ($rs->num_rows() > 0) {
			...
			foreach ($data as $each => $value) {
				...
				$row = $this->db->select('id,name_first,name_middle,name_last,user_type,gender')->where('id', $value['pituser'])->where('status_id', '1')->get('users')->row_array();
	...
	function get_user_pitchit_send_user_auth($wid) {
		$data = array();
		$usd = $this->session->userdata('logged_user');

		//$rs  = $this->db->select('id,name_first,name_middle,name_last')->from('users')->where('id', $wid)->get()->row_array();
		//$rs  = $this->db->select('s.id as work_id,s.user_id as work_user,s.title as work_title,a.id,a.name_first,a.name_middle,a.name_last')->from('works as s')->join('users as a','s.user_id = a.id','inner')->where('s.id', $wid)->get();

		$rs = $this->db->select('id,user_id')->from('works')->where('id', $wid)->get()->row_array();

		$rst = $this->db->select('id,name_first,name_middle,name_last,email,user_type')->from('users')->where('id', $rs['user_id'])->get()->row_array();
	...
	function get_pitchit_details_form_view() {
		...
		$rs  = $this->db->select('s.*,a.*')->from('work_pitchits as s')->join('works as a','s.wid = a.id','left')->where_not_in('s.pit_id', $temp)->order_by('s.created_date','desc')->get();*/
		$rs = $this->db->select('s.id as wfid,s.user_id as suid,s.work_type_id,s.work_form_id,w.*,p.pit_id,p.wid,p.pitchit,p.created_date,p.is_pitchit,p.allow_date')->from('publisher_forms as s')->join('works as w', 'w.work_type_id = s.work_type_id', 'left')->join('work_pitchits as p', 'p.wid = w.id', 'right')->where('s.user_id', $usd['id'])->where('p.allow_date >= now()')->order_by('p.pit_id', 'desc')->or_where('p.allow_date IS NULL')->where('s.user_id', $usd['id'])->order_by('p.pit_id', 'desc')->get();
	...
	function get_pitchit_details_form_view_author() {
		$data = array();
		$row_workid = array();
		$data44 = array();
		$temp = array();
		...
		/*$rs  = $this->db->select('s.*,a.*')->from('work_pitchits as s')->join('works as a','s.wid = a.id','left')->where_not_in('s.pit_id', $temp)->order_by('s.created_date','desc')->get();*/
		$rs = $this->db->select('w.*,p.pit_id,p.wid,p.pitchit,p.created_date,p.is_pitchit')->from('work_pitchits as p')->join('works as w', 'p.wid = w.id', 'right')->where('p.user_id', $usd['id'])->order_by('p.pit_id', 'desc')->get();
	...
	function get_pitchit_count() {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->where('is_deleted !=', '1')->where('is_drafted', '0')->where('is_moved', '0')->where('to_user_id', $usd['id'])->where('is_pitchited', "1")->where('is_notified', "0")->get('messages')->row_array();
	...
	function get_user_notification_count() {
		...
		$rs = $this->db->select('*')->where('to_user_id', $usd['id'])->where('pitchit_id', '')->where('is_notified', '0')->where('is_marked', '0')->get('messages');
	...
	public function update_pitchit_single_view($pit_id, $wid) {
		...
		// echo "sssssss";die;
		//$last_id=$this->db->insert_id();
		$rs = $this->db->select('count(*) as count')->where('wid', $wid)->where('user_id', $usd['id'])->get('pitchit_view')->row_array();
		...
		if ($type == 'reply') {
			...
			if (count($result) > 0) {
				foreach ($result as $key => $row) {
					...
					if ($this->input->post('pitchit_id') != '') {
						$data['pitchit_id'] = $this->input->post('pitchit_id');
					}
	...
	function pitchitCount() {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$rs = $this->db->select('count(*) as count')->where('is_deleted !=', '1')->where('is_drafted', '0')->where('is_viewed', '0')->where('is_moved', '0')->where('to_user_id', $usd['id'])->where('is_pitchited', '1')->order_by('created', 'desc')->get('messages');
	...
	function work_pitchitCount() {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$rs = $this->db->select('count(*) as count')->where('is_pitchit', '1')->get('work_pitchits');
	...
	function mailInfo($offset = null, $limit = null) {
		...
		if ($rs->num_rows() > 0) {
			...
			foreach ($data as $each => $value) {
				...
				$row = $this->db->select('id,name_first,name_middle,name_last,user_type,gender')->where('id', $value['from_user_id'])->where('status_id', '1')->get('users')->row_array();
				if (!empty($row['name_first'])) {
					$data[$each]['name'] = $row['name_first'];
				} else {
					$data[$each]['name'] = '';
				...
				$row22 = $this->db->where('user_id', $value['from_user_id'])->where('description', 'profile')->get('assets')->row_array();
				if (!empty($row22['filename'])) {
					$data[$each]['photo'] = $row22['filename'];
				} else {
					$data[$each]['photo'] = '';
	...
	function get_main_pitchit() {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$rs = $this->db->select('pit_id as id,user_id,wid,pitchit as subject,created_date as created,view,is_pitchit')->where('is_pitchit', '1')->get('work_pitchits');
		//die;
		//$this->db->limit(1,0);
		if ($rs->num_rows() > 0) {
			$data = $rs->result_array();
			...
			foreach ($data as $each => $value) {
				...
				$row25 = $this->db->select('id,asset_id')->where('id', $value['wid'])->get('works')->row_array();

				$row22 = $this->db->select('id,description,filename')->where('id', $row25['asset_id'])->where('description', 'cover')->get('assets')->row_array();
				if (!empty($row22['filename'])) {
					$data[$each]['cover_image'] = $row22['filename'];
				} else {
					$data[$each]['cover_image'] = '';
				...
				$row26 = $this->db->select('id,description,filename')->where('id', $row25['asset_id'])->where('description', 'profile')->get('assets')->row_array();
				if (!empty($row26['filename'])) {
					$data[$each]['photo'] = $row26['filename'];
				} else {
					$data[$each]['photo'] = '';
					//$data[$each]['type']    = 'PROPERTY';
				}
	...
	function get_main_org_pitchit() {
		...
		if ($rs->num_rows() > 0) {
			...
			foreach ($data as $each => $value) {
				...
				$row26 = $this->db->select('id,description,filename')->where('user_id', $value['user_id'])->where('description', 'profile')->get('assets')->row_array();
				if (!empty($row26['filename'])) {
					$data[$each]['photo'] = $row26['filename'];
				} else {
					$data[$each]['photo'] = '';
	...
	function singlePitchInfo($id) {
		...
		$data = $this->db->select('*')->where('pit_id', $id)->get('work_pitchits')->row_array();
	...
	function singleMailInfo($id) {
		...
		$rs = $this->db->select('s.*,s.created msg_created,a.id as uid,a.name_first,a.name_middle,a.name_last,a.gender,a.user_type')->from('messages as s')->join('users as a', 'a.id = s.to_user_id', 'left')->where('s.id', $id)->where('a.status_id', '1')->get();
		...
		if ($rs->num_rows() > 0) {
			...
			foreach ($data as $each => $value) {
				...
				$row22 = $this->db->select('user_id,description,filename')->where('user_id', $value['to_user_id'])->where('description', 'profile')->get('assets')->row_array();
				if (!empty($row22['filename'])) {
	...
	function toUser() {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$rs = $this->db->select('id,name_first,name_middle,name_last,user_type,gender')->where('id', $usd['id'])->where('status_id', '1')->get('users');
		//die;
		//$this->db->limit(1,0);
		if ($rs->num_rows() > 0) {
			$data = $rs->result_array();
	...
	function fromUser_pitch($id) {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$rs = $this->db->select('*')->where('pit_id', $id)->get('work_pitchits');
		//die;
		//$this->db->limit(1,0);
		if ($rs->num_rows() > 0) {
			$data = $rs->result_array();
			...
			foreach ($data as $each => $value) {
				...
				$row22 = $this->db->where('user_id', $value['user_id'])->get('assets')->row_array();
				if (!empty($row22['filename'])) {
					$data[$each]['photo'] = $row22['filename'];
				} else {
					$data[$each]['photo'] = '';
	...
	function dopitchit($id, $pit, $wid) {
		...
		$data['saved_count'] = $this->db->select('count(*) as count')->where('user_id', $usd['id'])->where('is_drafted', '1')->where('is_pitchit', '0')->get('work_pitchits')->row_array();

		return $data;

	}

	function dopitchit_cnt() {
		$usd = $this->session->userdata('logged_user');
		$data = array();

		$data = $this->db->select('count(*) as count')->where('user_id', $usd['id'])->where('is_drafted', '1')->where('is_pitchit', '0')->get('work_pitchits')->row_array();
	...
	function doCreatePitchit($id, $pitch, $pubid) {
		...
		if ($pitch != '') {
			...
			$hour_id = $this->db->select('*')->where('user_id', $usd['id'])->where('status', 'in')->get('pitch_transaction')->row_array();

			$today = date("Y-m-d H:i:s");
			$data22['allow_date'] = date('Y-m-d H:i:s', strtotime('+' . $hour_id['allow_hour'] . 'hours', strtotime($today)));
			$data22['allow_hour'] = $hour_id['allow_hour'];
			...
			$data43['subject'] = substr($pitch,0,20);
			$data43['body'] = $pitch;
			$data43['is_pitchited'] = '1';
			$data43['pitchit_id'] = $insert_id;
			...
			if ($row->num_rows() > 0) {
				...
				foreach ($row33 as $user_publisher) {
					...
					$from_user_name = $this->db->select('id,name_first,name_middle,name_last,status_id')->where('id', $usd['id'])->where('status_id', '1')->get('users')->row_array();

					$user_total_name = $from_user_name['name_first'] . ' ' . $from_user_name['name_middle'] . ' ' . $from_user_name['name_last'];

					$to_user_name = $this->db->select('id,name_first,name_middle,name_last,email,status_id')->where('id', $user_publisher['user_id'])->where('status_id', '1')->get('users')->row_array();

					$to_user_total_name = $to_user_name['name_first'] . ' ' . $to_user_name['name_middle'] . ' ' . $to_user_name['name_last'];

					$this->send_mail_to_registered_user_forPitchit($to_user_name['email'], $user_total_name, $to_user_total_name);
				}

				//echo $to_user_name['email'].' '.$user_total_name .' '.$to_user_total_name;die;
				return 1;
			}
	...
	function send_mail_to_registered_user_forPitchit($email, $user, $touser) {
  ...
  <tr style="background-color: #fff;">
    ...
    <p>You have a pitchit! message from ' . $user . '.To view the message please login into Inkubate</p>
...
/*************AEP MALE PERCENT*********************/
	...
	public function get_a_male_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '3')->where("gender", '1')->get()->row_array();

		return $data['count'];
	}
    public function get_e_male_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '4')->where("gender", '1')->get()->row_array();

		return $data['count'];
	}
    public function get_p_male_percent() {
		$usd = $this->session->userdata('logged_user');
...
/*************AEP FEMALE PERCENT*********************/
	...
	public function get_a_female_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '3')->where("gender", '2')->get()->row_array();

		return $data['count'];
	}
    public function get_e_female_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '4')->where("gender", '2')->get()->row_array();

		return $data['count'];
	}
    public function get_p_female_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '2')->where("gender", '2')->get()->row_array();
...
/*************AEP TOTAL PERCENT*********************/
	...
	public function get_a_total() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '3')->where('gender >', '0')->get()->row_array();
		
		/*print_r($data);die;*/
		return $data['count'];
	}
    
    public function get_e_total() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '4')->where('gender >', '0')->get()->row_array();
		
		/*print_r($data);die;*/
		return $data['count'];
	}
    public function get_p_total() {
		$usd = $this->session->userdata('logged_user');
...
/************* END AEP *********************/    
	...
	public function get_writer_male_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '1')->where("gender", '1')->where("is_exist", "1")->where("duplicate", "0")->get()->row_array();
	...
	public function get_writer_female_percent() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '1')->where("gender", '2')->where("is_exist", "1")->where("duplicate", "0")->get()->row_array();
	...
	public function get_writer_total() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '1')->where("gender !=", '')->where("is_exist", "1")->where("duplicate", "0")->get()->row_array();
	...
	function get_user_recently_add_titles($offset = null, $limit = null) {
		...
		if ($rs->num_rows() > 0) {
			...
			foreach ($data as $each => $value) {
				...
				$row = $this->db->select('name_first,name_middle,name_last,user_type,gender')->where('id', $value['user_id'])->get('users')->row_array();
				if (!empty($row['name_first'])) {
					$data[$each]['name_first'] = $row['name_first'];
	...
	function get_author_view_details($offset = null, $limit = null) {
		...
		if ($rs->num_rows() > 0) {
			...
			foreach ($data as $each => $value) {
				...
				$row = $this->db->select('name_first,name_middle,name_last,user_type,gender')->where('id', $value['wuid'])->get('users')->row_array();
				if (!empty($row['name_first'])) {
					$data[$each]['name_first'] = $row['name_first'];
	...
	public function get_latest_portrait($uid) {

		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('id,asset_guid,description,filename,user_guid,status_id')->from("assets")->where("user_id", $uid)->where("description", 'portrait')->where("status_id", '1')->order_by('id', 'desc')->limit(1)->get()->row_array();

		return $data;
	}

	public function get_latest_cover($uid, $wid) {

		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('id,asset_guid,description,filename,user_guid,status_id')->from("assets")->where("user_id", $uid)->where("id", $wid)->where("description", 'cover')->where("status_id", '1')->get()->row_array();
