...
class memail extends CI_Model {
		...
		if ($type == 'send') {
			...
			if (count($result) > 0) {
				foreach ($result as $key => $row) {
					...
					$this->send_mail_to_registered_user_forCompose($row['email'], $user_total_name, $to_user_total_name);
				}
			}
	...
	function send_mail_to_registered_user_forCompose($email, $user, $touser) {
  ...
  <tr style="background-color: #fff;">
    ...
    <p>You have a message from ' . $user . '.To view the message please login into Inkubate</p>
...
/*<p>Please <a href="http://billbahadur.com/demo/inkubate/bookshelves/sharebooklist/'.$bselfid.'">click here</a> to view</p>*/
	...
	function pitchitCount() {
		$data = array();
		$usd = $this->session->userdata('logged_user');
		$rs = $this->db->select('count(*) as count')->where('is_deleted !=', '1')->where('is_drafted', '0')->where('is_moved', '0')->where('to_user_id', $usd['id'])->where('is_pitchited', '1')->order_by('created', 'desc')->get('messages');
...
/*<p>Please <a href="http://billbahadur.com/demo/inkubate/bookshelves/sharebooklist/'.$bselfid.'">click here</a> to view</p>*/
	...
	public function get_aep_total() {
		$usd = $this->session->userdata('logged_user');
		$data = $this->db->select('count(*) as count')->from("users")->where("status_id", "1")->where("user_type", '2')->where('gender >=', '0')->get()->row_array();
		/*$data = $this->db->select('*')->from("users")->where("status_id", "1")->where("user_type", '2')->get()->result_array();
		echo "<pre>";
		print_r($data);die;*/
		return $data;
	}
