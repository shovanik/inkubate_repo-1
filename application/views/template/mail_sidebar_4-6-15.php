...
function removeEmail_draft(div,id)
{
	if(parseInt(id) > 0)
	{
		//alert(id);
		$("#name"+id).remove();
		var val = $("#user_email_id_draft").val();
		var arr = val.split(",");
		//console.log(arr);
		var index = arr.map(function(el) {
		  return parseInt(el);
...
function FnShowSearch_draft(value)
{
	$("#dropdown_search_draft").hide();
	if(value == "")
	{
		return false;
	}
                ...
                success:function(data){
                    ...
                    if(data.status == "1")
                    {
                    	...
                    	for (var i = 0, p; p = ps[i++];) 
                        {
                            ...
                            var name = p.name_first+" "+p.name_middle+" "+p.name_last;
                            html += '<li>';
                            html += '<a href="javascript:;" onclick="FnAddEmail_draft(\''+p.id+'\',\''+name+'\')">'+name+'</a>' ;
                            
                            
		            html += '</li>';
		            html += '<div class="clear"></div>';
                            $("#dropdown_search_draft").append(html);                  
                        }
                        $("#dropdown_search_draft").show();
                        }
...
function FnAddEmail_draft(id,name)
{
...
.save_but{
    background: none repeat scroll 0 0 #d0e6fe;
...
<div class="cd-popup" role="alert">
    ...
    <div style=" width:100%; background:#000; padding:20px 0;">
		   ...
		   <div class="top_compose">
						...
						<a href="javascript:void(0);"><img src="<?=base_url()?>images/delete_new.png" alt="" />
						</a>
					    </div>
  ...
  <tr>
    <td width="54%"><span href='javascript:;' onclick='folder_message(<?php echo $fdetails['id']?>,"<?php echo htmlspecialchars($fdetails['name'],ENT_QUOTES)?>","folder")'> 
        <?php
        if(strlen($fdetails['name']) > 12)
            {?>
        <label style="cursor:pointer" title="<?php echo ucfirst($fdetails['name']);?>"><?php echo substr($fdetails['name'], 0, 10)."..";?></label>
               
            <?php }
            else
            {
                echo ucfirst($fdetails['name']);
            }
            ?>
    ...
    <td width="22%"><span href="javascript:;" onclick="delete_folder('<?php echo $fdetails['id']?>', 'folder');" style="cursor:pointer;">
                        ...
                        <table width="100%" border="0">
  <tr>
    <td width="54%"><span href="javascript:;" onclick='folder_message(<?php echo $pdetails['id']?>,"<?php echo $pdetails['name']?>","pitchit")'>
                            <?php //echo ucfirst($rest);?>
        <?php
        if(strlen($pdetails['name']) > 12)
            {?>
        <label style="cursor:pointer" title="<?php echo ucfirst($pdetails['name']);?>"><?php echo substr($pdetails['name'], 0, 10)."..";?></label>
               
            <?php }
            else
            {
                echo ucfirst($pdetails['name']);
            }
            ?>                    
    </span></td>
