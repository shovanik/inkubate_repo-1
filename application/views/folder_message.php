 ...
 <script>
                    ...
                    <div class="mid_content_inner_right">
                        ...
                        $(document).ready(function() {
                           ...
                           $('#move_inbox').click(function(){
                                ...
                                {
                                    $.ajax({
                                        url      : '<?=base_url()?>'+'home/moveToInbox',
                                        type     : 'POST',
                                        data     : { 'id':favorite, 'msg_type' : '<?php echo $msg_type;?>' },
                                        success  : function(resp){
                                            if(resp.success == 1)
                                            {
                                                html = "<p style='color:green;'>Mail successfully added to inbox</p>";
                                    ...
                                    <div class="drop_cont" style="margin-left: 10px;">
                                        <input type="hidden" name="hide_foldid" id="hide_foldid" value="<?php echo $this->uri->segment(3);?>">
                                        <ul class="dropdown folder">
                                            <h2 class="dropdown-title">Move to</h2>
                                            <li class="dropdown-list folderlist" id="move_inbox">Inbox</li>
                                    <?php 
                                    if($msg_type == 'pitchit'){
                                    ...
                                    $(document).ready(function() {
                                       ...
                                       $('#fold_<?php echo $fdetails['id']?>').click(function(){
                                            ...
                                            if(favorite.length > 0)
                                                   {
                                                     $.ajax({
                                                       url      : '<?=base_url()?>'+'home/moveToFolder',
                                                       type     : 'POST',
                                                       data     : { 'id':favorite , 'foldId' : '<?php echo $fdetails['id']?>' , 'foldname' : '<?php echo $fdetails['name']?>', 'msg_type' : '<?php echo $msg_type;?>'  },
                                                       success  : function(resp){//alert(resp.status);return false;
                                                            /*if(resp != '0'){
                      ...
                      {
                    ?>    
                   <a href="<?=base_url()?>discovery/user_details/<?php echo $details['from_user_id'];?>" class="tooltips">
                   <?php 
                   if(strlen($details['name']) > 15)
                    {?>
                <label style="cursor:pointer" title="<?php echo $details['name'];?>"><?php echo substr($details['name'],0,15).'..';?></label>
                   ...
                   <!-- <span class="tp_span55"> <?php echo $details['name']; ?></span>
                    <div class="clear"></div> -->
        ...
        <a href="#" class="mail_link" style="width:16px;">&nbsp;</a>
         ...
         <?php } ?>      </span> 
                        ...
                        <span class="soph_con1 detail_for_mob"><a class="mail_link" href="<?=base_url()?>mail/details/<?php echo $details['message_id']?>">
                            <?php
