...
function feedurl_status(id)
 ...
 function feed_url_id()
 {
   ...
   //alert(id);
    $.ajax({
        url      : '<?=base_url()?>'+'feeds/feedurl_status_author',
        type     : 'POST',
        data     : { 'id': id },
...
if (isset($_REQUEST['mywork']) && $_REQUEST['mywork'] == '1') {?>
...
//echo '<pre/>';echo "uploadImage/".$usd['id']."/profile/".$user_photo['filename'];die;
//echo "_assets/".strtolower($latest_portrait['user_guid'])."/".strtolower($latest_portrait['asset_guid']).".jpg";die;
?>
...
if (strlen(trim($fullname)) > 15) {?>
                 <label style="cursor:pointer" title="<?php echo trim($fullname);?>"><?php echo substr(trim(ucwords($fullname)), 0, 15) . '..';?></label>
...
?>
        ...
        <div class="dasbord_news_section">
          ...
          <div class="feed_loader_file"></div>
            ...
            <span id="feeds_pop" style="cursor: pointer;">
            <img src="<?=base_url()?>images/pro_edit.png" alt="" />
            </span>
            </h1>
...
?>
            <div class="<?php if (!empty($feeds_details) && count($feeds_details) > 4) {?>demo1<?php } elseif (!empty($pitchit_details) && count($pitchit_details) > 4) {?> demo1 <?php } else {?> noticker <?php }
?> demo3 demo_new">
                <ul>
                    <?php
...
<?php } ?>
        ...
        <div class="pitchits_section pitchits_section_new">
          <div class="pitchits_section_left" id="my_pitchit">
     ...
     <div class="clear"></div>
			...
			if (strlen($pitch_details['pitchit']) > 22) {
				?>
                  ...
                  <?=substr($pitch_details['pitchit'], 0, 22) . '..'?>
                  </span>
...
$fullname = $pitchit_view_user['name_first'] . ' ' . $pitchit_view_user['name_middle'] . ' ' . $pitchit_view_user['name_last'];
			if (strlen($fullname) > 10) {?>
                <div title="<?php echo $fullname;?>"><?php echo substr($fullname, 0, 10) . '..';?></div>
                ...
                var p;
                    ...
                    if(data.status == "true")
                    {
                        ...
                        for (var i = 0, p; p = ps[i++];)
                        {
                         ...
                         if(p.pitchit.length > 22)
                         {
                           var trimmedString = p.pitchit.substring(0, 22)+'..';
                           $("#pit_work_vw88_"+id).html(trimmedString);
                           $("#pit_work_vw88_"+id).attr('title',p.pitchit);
                         }
                         else
                         {
    ...
    if(confirm("Congratulations! You are about to send this Pitchit! This cannot be undone. Are you sure you are ready to do this…did you check your spelling")){
         $.ajax({
           ...
           success  : function(data){
               ...
               //alert(data.saved_count);
               //var pitcnt = resp.split('~');
               //var data = pitcnt[0];
                var p;
                    var ps = data.messages;
                    var html='';
                    //var pg = parseInt(page) + 1;
                    console.log(ps.saved_count.count);
                    //console.log(data);
                    //alert(data.status);
                    //var count = parseInt(data['count']);
                    if(data.status == "true")
                    {
                        ...
                        $("#pit_saved").text(ps.saved_count.count);
       ...
       success  : function(resp){
        //alert(resp);
            if(resp != '0'){
                ...
                //$("#edit_pit_save").html(html);img_grn_
                $("#pit_save_img_"+id).attr('src','<?=base_url()?>images/icon033.png');
                $("#img_grn_"+wid).attr('src','<?=base_url()?>images/icon033.png');
                $("#success_pit_"+id).html('Succesfully Pitchited!');
                $("#pit_saved").text(rsp[0]);
               // window.location.href = '<?//=base_url()?>home/author';
                $('.txtAge').hide();
                $(".demo").fancyfields();
	...
	foreach ($user_pitchit_details as $pitch_details) {
		...
		?>
              ...
              <tr>
                  ...
                  <?//=$pitch_details['title']?>
                  <?php
                if (strlen($pitch_details['title']) > 17) {?>
                              	<div title="<?php echo $pitch_details['title'];?>"><?php echo substr($pitch_details['title'], 0, 17) . '...';?></div>
		...
		?>
                  ...
                  <?php
                    if (!empty($pitch_details['pvuid'])) {
                    			...
                    			?>
                                        <?php 
                               $full_name5 = $pitch_details['name_first'] . ' ' . $pitch_details['name_middle'] . ' ' . $pitch_details['name_last'];
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_new">
              ...
              <tbody>
                <tr>
                  <td colspan="5">
                  <div class="pad_20">
                    <h3>You have <?php if ($purchase_pitchit['sum_total'] > 0) {echo ($purchase_pitchit['sum_total'] - $pitchit_use_count['count']);} else {echo '0';}
?> available PitchIts!</h3>
                   <p>0 are earned PitchIts!</p>
                   <p><?php echo $pitchit_use_count['count']?> Pitchits are done</p>
                   <p><?php if ($purchase_pitchit['sum_total'] > 0) {echo $purchase_pitchit['sum_total'];} else {echo '0';}
?> are purchased PitchIts!</p>
		   ...
		   success  : function(resp){
		    //alert(resp);
                ...
                else
                {
                    ...
                    alert('Please complete your previous package!');
                }
		        //$("#content_part").unblock();
		   },
		   error    : function(resp){
...
?>
                            ...
                            <input name="button" type="button" value="Checkout" style="margin-right:31px;" class="green_but flo_right" id="checkout" onclick="getPurchaseDiv();" /></td>
                            </tr>
                            </table>
          ...
          <!--<a href="#" class="button_pro">View Detail</a>--></span></h1>
            ...
            <div class="publishers_section_left_pad">
              <ul>
                ...
                /*echo $aep_male_percent;
                echo $aep_female_percent;
                echo $total_aep; */
                
                ?>
                            <?php $male = ($aep_male_percent * 100) / $total_aep;
            //echo $m = sprintf ("%.2f", $male);
            echo round($male);
            ?>% Female AEPs</li>
                            <li><img src="<?=base_url()?>images/male.png" alt="" />
                            <?php $female = ($aep_female_percent * 100) / $total_aep;
...
if (!empty($user_view_details)) {
  ...
  <tr>
    ...
    <a class="tooltips" href="javascript:void(0);">
    <div>
    <?php if(!empty($total_viewer['title'])) { ?>
    <?=(strlen($total_viewer['title']) > 20 ? '<div title="' . $total_viewer['title'] . '">' . substr($total_viewer['title'], 0, 20) . '...</div>' : $total_viewer['title']);?>
    <?php } else { echo 'No title viewed'; } ?>
    </div>
...
//print_r($user_work_details);die;
	...
	foreach ($user_work_details as $key => $details) {
		...
		$latest_cover = $this->memail->get_latest_cover($usd['id'], $details['asset_id']);

		//echo '<pre/>';print_r($latest_cover);

		?>

                <div class="item" id="book<?php echo $details['id'];?>">
                <h4 style="margin-top:12px; cursor: pointer;" title="<?=$details['title']?>"><?php if (strlen($details['title']) > 18) {?><?=substr($details['title'], 0, 18)?>... <?php } else {?><?=$details['title']?><?php }
		?></h4>

                <div class="item_left_section" style="width:85px; height:104px;overflow: hidden;">
                ...
                <?php } elseif (file_exists("_assets/" . strtolower($latest_cover['user_guid']) . "/" . strtolower($latest_cover['asset_guid']) . ".jpg")) {?>
            <img src="<?=base_url()?>_assets/<?=strtolower($latest_cover['user_guid'])?>/<?=strtolower($latest_cover['asset_guid']) . '.jpg'?>" class="img_sz"/>
...
/*$title = "";
		...
		?>
                ...
                <!--<h4><?php if (strlen($details['title']) > 18) {?><?=substr($details['title'], 0, 18)?>... <?php } else {?><?=$details['title']?><?php }
		?></h4>--><br>
...
$full_name = $details['name_first'] . ' ' . $details['name_middle'] . ' ' . $details['name_last'];
		...
		?>
				<p>By <span><?php echo trim($full_name);?></span></p>
	...
	?>
               ...
               <?php } else {?>
                ...
                <div style="margin: 80px 0 0 240px;font-size: 19px;">
                <p>No works are added yet,</p>
                <p>To add a work please click on Add/Edit titles.</p>
                </div>
           ...
           success  : function(resp){
           		...
           		setTimeout( function(){
				    if(resp == 1)
                    {
                        ...
                        window.location.href = '<?=base_url()?>home/author#my_pitchit';
                    }
                    else
                    {
               ...
               // var res = resp.split("~");
                    if(resp != '0')
                    {
                       ...
                       var rsp = resp.split('~');
                        ...
                        $("#content").find("[id^='tab']").hide(); // Hide all content
                        $("#tabs li:first").attr("id",""); // Activate the first tab
