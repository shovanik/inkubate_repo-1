  ...
  jQuery.ias({
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_new">
        ...
        <tbody id="book_shelf">
          ...
          <?php 
               //echo '<pre/>';print_r($writers_category);die;
               if(!empty($writers_category))
                {
                $i =1;    
                 foreach($writers_category as $details)
                 {
          ...
          <tr class="itemld">
            ...
            <img src="<?=base_url()?>uploadImage/<?=$details['user_id']?>/profile/thumbs/<?=$details['photo']?>"/>
            <?php } else { ?>
            ...
            <p class="parlf"><span><?php echo substr(html_entity_decode($details['biography'], ENT_QUOTES, 'UTF-8'),0,50).'...';?></span></p>
           
            <p class="parlf"><strong>works</strong>: 
            <?php 
            $writer_work = $this->mbookshelf->get_writer_work($details['user_id']);
