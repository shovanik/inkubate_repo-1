...
var BASE = '<? echo base_url()?>';
</script>

<script type="text/javascript" src="<?=base_url()?>js/mail_details.js"></script>
   ...
   .mid_content_inner_right_bottom_box_new {
...
<span>
    ...
    <a href="#0"  class="reply-popup-trigger" href="javascript:void(0)" onclick="reply_send('<?php echo $this->uri->segment(3)?>')"><input name="button" type="button" value="" class="button01" alt="reply" title="reply"/></a>
    ...
    $reply_user = $this->memail->reply_to_user($single_mail_details[0]['from_user_id']);
    //echo '<pre/>';print_r($single_mail_details);die;
                ...
                <div class="top_reply">
                   ...
                   <form name="replyFrm" id="replyFrm" method="post" action="<?php echo base_url();?>mail/replybox" enctype="multipart/form-data"> 
                    ...
                    <div class="text_field_box">
                        ...
                        <input type="hidden" id="msg_type" name="msg_type" value="<?php echo $single_mail_details[0]['is_pitchited']?>"/>
                        <input type="hidden" id="pitchit_id" name="pitchit_id" value="<?php echo $single_mail_details[0]['pitchit_id']?>"/>
                        ...
                        <input type="hidden" id="user_mail_reply" name="user_mail_reply" readonly="readonly"/>
                        <div  class="auto_main">				
                            <span id="email_selected_reply">
<!--                                <span class="choosen" id="fullname"></span>-->
                            </span>
                            
                            <span>
                                <input type="text" autocomplete="off" class="auto_t_box" id="email_input_reply" name="email_input_reply" onkeyup="FnShowSearch_reply(this.value, 'reply')">
                                <ul id="dropdown_search_reply" class="dropdown_search_reply" style="display:none;">
                                </ul>
                            </span>
                           
                        </div>
                        <input type="hidden" id="user_email_id_reply" name="user_email_id_reply"/>
                        <label class="com_plus"> <a href="#showAuthorList_reply" class="nohover add_img" rel="facebox" data-url=""><img src="<?php echo base_url('images/plus_icon.png')?>" /></a></label>
                    ...
                    <div class="text_field_box3"><label style="float:left; margin-top:9px;">Subject</label>
                        <input type="text" id="reply_sub" name="reply_sub"  class="sub_mail_content" value="<?php echo $single_mail_details[0]['subject']?>" >
                        <div class="clear"></div>
                    </div>
...
<span>
    <a href="#0"  class="forward-popup-trigger" onclick="forword_send()"><input name="button" type="button" value="" class="button05" style="border-right: 0;" alt="forward" title="forward"/></a>
...
<span>
<a href="#0"  class="replyall-popup-trigger"><input name="button" type="button" value="" class="button06" style=" width: 10%;" alt="reply to all" title="reply to all"/></a>
...
<div class="drop_cont2" style="margin-left: 10px;">
                                        ...
                                        <ul class="dropdown2 folder">
                                            <!--<input name="button" type="button" value="" class="button07" style="border-right: 0;" />-->
                                           <img src="<?=base_url()?>images/icon_52.png" style="border-right: 0; margin-top: 8px; cursor: pointer;" alt="move to folder" title="move to folder"/>
                                    <?php 
                                    ...
                                    $(document).ready(function() {
                                       ...
                                       $('#fold_<?php echo $fdetails['id']?>').click(function(){
                                            ...
                                            //alert(msg_type);
                                              var favorite = [];
                                              favorite.push(<?php echo $single_mail_details[0]['id']?>);
                                              //var favorite = [];
...
<span><input name="button" type="button" value="" class="button08" style="border-right: 0;" onclick="draft_details(<?php echo $this->uri->segment(3);?>)" alt="save as draft" title="save as draft"/></span>
<span><input name="button" type="button" value="" class="button04" style="border-right: 0;" id="dt" alt="delete" title="delete"/></span>
...
<div class="clear"></div>
   ...
   <div class="mid_content_inner_right_top pad_top10 bor_bot pad_bot10">
       ...
       <div class="cb-styles" style="float:left; margin-left:1px; margin-right:11px;">
    
              <input type="checkbox" id="checkbox67" name="check1" class="css-checkbox lrg" value="ALL">
              <label for="checkbox67" name="checkbox67_lbl" class="css-label lrg web-two-style check1"></label>
                    
       </div>

       <span id="re_id">
       
       <?php echo $single_mail_details[0]['subject'];?> 
...
if(!empty($reply_mail_details)){
 ...
 <?php /*<p class="right_date"><span>Date </span>: <strong><?php echo date('m/d/y',strtotime($reply_data['msg_created']))?></strong></p> */?>
 <p class="right_date"><span><?php echo date('F d Y',strtotime($reply_data['msg_created'])) ?> </span> at <strong><?php echo date('h:i A',strtotime($reply_data['msg_created']))?></strong></p>
            ...
            <input type="checkbox" id="checkbox67_<?php echo $reply_data['id'];?>" name="check" class="css-checkbox lrg check1" value="<?php echo $reply_data['id'];?>">
            <label for="checkbox67_<?php echo $reply_data['id'];?>" name="checkbox67_lbl" class="css-label lrg web-two-style check1"></label>
    ...
    <?php if(!empty($reply_data['filename'])) { ?>
                ...
                <?php } elseif($reply_data['gender'] == "1") { ?>
                        <img src="<?=base_url()?>images/man_large.jpg" style="border: 1px solid #444;" class="img_sz_small"/>
                        
            <?php } elseif($reply_data['gender'] == "2") { ?>
                        <img src="<?=base_url()?>images/woman_large.jpg" style="border: 1px solid #444;" class="img_sz_small"/>            
             <?php } elseif($reply_data['user_type'] == "1") { ?>
                        <img src="<?=base_url()?>images/ico_writers1.png" style="border: 1px solid #444;" class="img_sz_small"/>  
                    <?php }else{?>
                        <img src="<?=base_url()?>images/ico_publishers1.png" style="border: 1px solid #444;" class="img_sz_small"/>
          <?php }?> 
               ...
               <?php /* <p><strong>Re : <?php echo $reply_data['subject']?></strong></p> */?>
                ...
                <br />
                <span>To</span>: <strong><?php echo trim($reply_data['name_first'].' '.$reply_data['name_last']);?> </strong>
                <!--<img src="<?//=base_url()?>images/add_con.png" alt="" />-->
                </p>
...
<?php
if($single_mail_details[0]['in_replied_section']==0)
{
?>
<div class="mid_content_inner_right_bottom_box_new">
<?php /* <p class="right_date"><span>Date </span>: <strong><?php echo date('m/d/y',strtotime($single_mail_details[0]['msg_created']))?></strong></p> */?>
<p class="right_date"><span><?php echo date('F d Y',strtotime($single_mail_details[0]['msg_created']))?> </span> at <strong><?php echo date('H:i A',strtotime($single_mail_details[0]['msg_created']))?></strong></p>
<div class="drop_menu_section_left">
<div class="prof">
    <div class="drop_menu_section_left_text">
        ...
        <input type="checkbox" id="checkbox67_<?php echo $single_mail_details[0]['id'];?>" name="check" class="css-checkbox lrg check1" value="<?php echo $single_mail_details[0]['id'];?>">
        <label for="checkbox67_<?php echo $single_mail_details[0]['id'];?>" name="checkbox67_lbl" class="css-label lrg web-two-style check1"></label>
                    
        </div>
                


     <?php //echo '<pre/>';print_r($from_user);die;
    ...
    <?php if(!empty($from_user_details['photo'])) { ?>
                <img src="<?=base_url()?>uploadImage/<?=$from_user_details['from_user_id']?>/profile/<?=$from_user_details['photo']?>" alt="" class="img_sz_small" />
                 <?php } elseif($from_user_details['gender'] == "1") { ?>
                        <img src="<?=base_url()?>images/man_large.jpg" style="border: 1px solid #444;" class="img_sz_small"/>
                        
            <?php } elseif($from_user_details['gender'] == "2") { ?>
                        <img src="<?=base_url()?>images/woman_large.jpg" style="border: 1px solid #444;" class="img_sz_small"/>            
             <?php } elseif($from_user_details['user_type'] == "1") { ?>
                        <img src="<?=base_url()?>images/ico_writers1.png" style="border: 1px solid #444;" class="img_sz_small"/>  
                    <?php }else{?>
                        <img src="<?=base_url()?>images/ico_publishers1.png" style="border: 1px solid #444;" class="img_sz_small"/>
          <?php }?> 
    ...
    <div class="clear"></div>
    <?php if(!empty($single_mail_details[0]['filename'])) { ?>
                <img src="<?=base_url()?>uploadImage/<?=$single_mail_details[0]['to_user_id']?>/profile/<?=$single_mail_details[0]['filename']?>" alt="" class="img_sz_small" />
                <?php } elseif($single_mail_details[0]['gender'] == "1") { ?>
                        <img src="<?=base_url()?>images/man_large.jpg" style="border: 1px solid #444;" class="img_sz_small"/>
                        
            <?php } elseif($single_mail_details[0]['gender'] == "2") { ?>
                        <img src="<?=base_url()?>images/woman_large.jpg" style="border: 1px solid #444;" class="img_sz_small"/>            
             <?php } elseif($single_mail_details[0]['user_type'] == "1") { ?>
                        <img src="<?=base_url()?>images/ico_writers1.png" style="border: 1px solid #444;" class="img_sz_small"/>  
                    <?php }else{?>
                        <img src="<?=base_url()?>images/ico_publishers1.png" style="border: 1px solid #444;" class="img_sz_small"/>
          <?php }?>
    ...
    <div class="right_tex">
    <p class="right_tex"><span>From</span>: <strong><?php echo trim($from_user_details['name'].' '.$from_user_details['name_last']); ?></strong>
    <!--<img src="<?=base_url()?>images/add_con.png" alt="" />--><br />
    <span>To</span>: <strong><?php echo trim($single_mail_details[0]['name_first'].' '.$single_mail_details[0]['name_last']);?> </strong><!--<img src="<?=base_url()?>images/add_con.png" alt="" />--><br /></p>
    <!--<p><span>Subject </span>: &nbsp;<strong><?php //echo $single_mail_details[0]['subject'];?></strong></p>-->
...
<?php if(!empty($single_mail_details[0]['attach_file'])) {?>
<a href="<?=base_url()?>mail/download/<?=$single_mail_details[0]['from_user_id']?>/<?=$single_mail_details[0]['attach_file']?>" class="attachment_icon"><img alt="" src="<?=base_url()?>images/attachment_icon.png" title="<?php echo $single_mail_details[0]['attach_file'];?>"/><?php //echo $single_mail_details[0]['attach_file'];?></a>
<?php } ?>
</div>
<div class="clear"></div>
</div>
...
/*<div class="comment">
<?php if($single_mail_details[0]['is_pitchited'] == '1') {?>
<p>View the Pitchit : <a href="<?php echo $single_mail_details[0]['body'];?>">click here</a></p>
<?php } else {?>
<p><?php echo $single_mail_details[0]['body'];?></p>
<?php } ?>
</div>*/?>
		...
		if ($(this).val() !='ALL') {
		   ...
		   uncheckval.push($(this).val());
            ...
            if(ids.length > 0){
                ...
                var val = ids.join("-");		
                var mid = '<?php echo $this->uri->segment(3);?>';
		if (mid==val)
		{
		   if(uncheckval.length>0)
		   {
			window.location.href = '<?=base_url()?>mail/msgDeleteFromList/'+mid+'/'+val+'/'+ids1;
		   }
		   else
		   {
			window.location.href = '<?=base_url()?>mail/msgDelete/'+mid+'/'+val+'/'+ids1;
		   }
		}
		else
		{
		    window.location.href = '<?=base_url()?>mail/msgDelete/'+mid+'/'+val+'/'+ids1;
                ...
                alert("Please select checkbox to delete message.");
                return false;
            }
        } else {
           return false; 
