<?=$this->load->view('template/inner_bookshelf_header.php');

?>
<?php $usd = $this->session->userdata('logged_user');
//echo '<pre/>';print_r($share_bookshelf_with_count);die;
//$publisher = $this->mbookshelf->get_publishers();
//echo $publisher;
?>
<script type="text/javascript" src="<?=base_url()?>js/jquery-ui.min.js"></script>

<!--<link rel="stylesheet" type="text/css" href="<?//= base_url()?>js/src/jquery.tagsinput.css" />
<script type="text/javascript" src="<?//= base_url()?>js/src/jquery.tagsinput.js"></script>-->


<!-- Add required fancyBox files -->
<link rel="stylesheet" href="<?=base_url()?>js/fancybox/source/jquery.fancybox.css" type="text/css" media="screen" />
<script type="text/javascript" src="<?=base_url()?>js/fancybox/source/jquery.fancybox.pack.js"></script>
...
<script>
				...
				$('#select-to').selectize({
                    ...
                    <?php
$publisher = $this->mbookshelf->get_publishers();
if (isset($publisher) && count($publisher) > 0) {
	foreach ($publisher as $eachList) {
		?>

     {email: '<?php echo $eachList['email']?>', first_name: '<?php echo $eachList['name_first']?>', last_name: '<?php echo $eachList['name_last']?>'},
                        <?php }}
?>
					],
					render: {
...
<script>
    ...
    function ajaxDiscovery_with(page)
	{
			//$("#content_part").block();
	 		//type = "format";
	 		//var format_str = arr_format.join(",");
	        ...
	        var uid;
	 		...
	 		$.ajax({
			   url      : '<?=base_url()?>'+'bookshelves/ajax_bokkshelf_with',
			   type     : 'POST',
			   data     : { 'page': page},
			   success  : function(resp){
			    //alert(resp);
      ...
      function del_share(id)
	  {
	        ...
	        //alert(id);
	        if(document.getElementById("check_"+id).checked == true){
	        if(confirm('Are you sure to delete this shared title?'))
	        {
	             $.ajax({
	               url      : '<?=base_url()?>'+'bookshelves/delete_bookshelf_share_with',
	               type     : 'POST',
	               data     : { 'id': id },
	               success  : function(resp){
	                //alert(resp);
      ...
      function del_share_by(id)
	  {
	        ...
	        {
	             $.ajax({
	               url      : '<?=base_url()?>'+'bookshelves/delete_bookshelf_share_by',
	               type     : 'POST',
	               data     : { 'id': id },
	               success  : function(resp){
	                //alert(resp);
	  ...
	  $(document).ready(function(){
	    ...
	    $('#tab_book1').click(function(){
           ...
           $('#tab_book2').css('color','none'); */
            ...
            $('#tab_book4').css('background','#fff');
            $('#tab_book4').css('color','#000');

            $('#tab_book5').css('background','#fff');
            $('#tab_book5').css('color','#000');
        ...
        $('#tab_book4').click(function(){

           $('#tab_book1').css('background','#fff');
           $('#tab_book1').css('color','#000');

            $('#tab_book2').css('background','#fff');
            $('#tab_book2').css('color','#000');
       ...
       $('.view_tab4').click(function(){

        $('#tab_book4').css('background','#46b8ff');
        $('#tab_book4').css('color','#fff');

       })

       $('.view_tab5').click(function(){

        $('#tab_book5').css('background','#46b8ff');
        $('#tab_book5').css('color','#fff');
...
.fancybox-inner{ overflow: hidden; height:300px!important;}
.fancybox-skin { min-height:240px!important;}
...
.chosen-container {width: 75% !important;}
</style>
...
<div class="mid_content index_sec">
	<div class="pitchits_section pitchits_section_new pitchits_section_new3 pitchits_section_new4">
        ...
        <div class="pit_work_dialog_lat_first popup_chng" style="display: none;" id="pit_work_dialog">
			...
			<a href="javascript:void()" class="pitchit_pop_icon" onclick=""><span id="pitchit_img_icon"></span></a>
            <a href="javascript:void()" class="reply-popup-trigger think_pop_icon" id="conversationMail" onclick="">
            <span id="conversation_img_icon"></span></a>
     ...
     <div class="pit_work_dialog_lat_first_share popup_chng" style="display: none;" id="pit_work_dialog">

			<h4 id="title2"></h4>
			<p ><span id="tags2"><span><br>
            ...
            <a href="javascript:void()" class="reply-popup-trigger think_pop_icon" id="conversationMail" onclick="">
            <span id="conversation_img_icon"></span></a>-->
			<a  class="blue_but" href="#bookshelf_add_1" rel="facebox">Add to Bookshelf</a>
            <a href="javascript:void(0);" id="cancl_pit_share" class="green_but">Close</a>
		</div>
     ...
     <!---------------End----------------------------->
		...
		<div class="filter_section">
			<form action="" method="post" onsubmit="return false;">
                ...
                <div class="bookself_hright">
                <ul class="tabs_new">
                <li><a href="#" name="tab1" id="tab_book11">My Total Bookshelves (<?php echo $bookshelf_count;?>)</a></li>
                <li><a href="#" name="tab4" id="tab_book44">Bookshelves Shared With Me (<?php echo $share_bookshelf_test_count;?>)</a></li>
                <li><a href="#" name="tab5" id="tab_book55">Bookshelves I Have Shared (<?php echo $share_bookshelf_with_total;?>)</a></li>
                </ul>
                </div>

				<!--<div class="filter_section_search_results">My Total Bookshelves (<?php //echo $bookshelf_count;?>)</div>
                <div class="filter_section_search_results">Bookshelves Shared With Me(<?php //echo $bookshelf_count;?>)</div>
                <div class="filter_section_search_results">Bookshelves I Have Shared (<?php //echo $bookshelf_count;?>)</div>-->
		...
		<div class="pitchits_section_left">
			<div id="navigation_vert">
				...
				<ul class="list tabs" id="tabs">
                   ...
                   <!-- <li><a href="#" name="tab4" id="tab_book4"><img src="<?//= base_url()?>images/s_icon02.png" alt="" /> Shared By Bookshelves</a></li>
                    <li><a href="#" name="tab5" id="tab_book5"><img src="<?//= base_url()?>images/s_icon02.png" alt="" /> Shared With Bookshelves </a></li>-->
				</ul>
			</div>
		</div>
	...
	foreach ($bookshelf_test as $details) {
		...
		?>
                                ...
                                <td class="center" id="detail<?php echo $details['id'];?>">
                                 ...
                                 <?php $bkslf22 = str_replace("'", "`", $details['name']);?>

                                 <a href="#myDiv" class="various button_pro" onclick='share_book("<?php echo $details['id'];?>","<?php echo $bkslf22;?>")'>Share</a>
...
if ($total_rows > 5) {
	...
	?> <?php if ($total_rows > $offset) {?><a href="javascript:;" onclick="ajaxDiscovery(<?php echo $page + 1;?>)" class="blue_but">VIEW MORE</a><?php } else {?><a href="javascript:;" class="blue_but">VIEW MORE</a> <?php }
	?></div>
								</td>
							</tr>
                           <?php }
...
$full_name = $total_work['name'] . ' ' . $total_work['name_middle'] . ' ' . $total_work['name_last'];
			if (strlen($full_name) > 15) {
				$full_name = '<a href="javascript:void(0);" class="tooltips" style="font-size:11px;" title="' . $full_name . '">' . substr($full_name, 0, 12) . '...</a>';
			} else {
				$full_name = $full_name;
			}
...
if (!empty($saved_title_bookshelf)) {
	foreach ($saved_title_bookshelf as $key => $total_work_saved) {
		$total_name = $this->mbookshelf->get_user($total_work_saved['user_id']);
		$full_name_saved = $total_name['name_first'] . ' ' . $total_name['name_middle'] . ' ' . $total_name['name_last'];
		$latest_cover = $this->memail->get_latest_cover($total_work_saved['user_id'], $total_work_saved['asset_id']);
		?>
...
echo substr($full_name_saved, 0, 7) . '..';
		...
		?>
            ...
            <p>Format:
            	<?php
if (strlen($total_work_saved['type_name']) > 10) {?>
                     	<span title="<?php echo $total_work_saved['type_name'];?>"><?php echo substr($total_work_saved['type_name'], 0, 10) . '...';?></span>
                    <?php } else {
		...
		?>
            ...
            <p>Genre:
            	<?php
if (strlen($total_work_saved['form_name']) > 10) {?>
                     	<span title="<?php echo $total_work_saved['form_name'];?>"><?php echo substr($total_work_saved['form_name'], 0, 10) . '...';?></span>
                    <?php } else {
			echo $total_work_saved['form_name'];
		}
		?>
    		</p>
    <a href="javascript:;" class="blue_but view_open" onclick="openDialog(<?php echo $total_work_saved['Wid'];?>)" style="font-size: 10px;">VIEW</a>
    <a href="javascript:void(0);" class="green_bg" onclick="del_savetitle(<?php echo $total_work_saved['bid'];?>,<?php echo $total_work_saved['wcid'];?>)" style="font-size: 10px;">DELETE</a>
                                </div>
                        </div>
                        <?php }} else {?>
	...
	foreach ($share_bookshelf_test as $details_r) {
		...
		?>
							<tr id="bookshelve<?php echo $details_r['share_id'];?>">
								<td align="center">
                                <p class="ali_text" title="<?php echo $details_r['name'];?> was shared with you by <?php echo $full;?>" style="cursor: pointer;">
                                <?php echo $details_r['name'];?><br />
                         <?php /*<span title="<?php echo $full;?>" style="cursor: pointer;">shared by <?php echo substr($full,0,15);?></span> */?>
                                </p>
		...
		?>
								...
								<td class="center" id="detail<?php echo $details_r['share_id'];?>">
									<?php /*<a href="<?=base_url()?>bookshelves/booklist/<?php echo $details['id'];?>"><img alt="" src="<?=base_url()?>images/list_view.png" /></a>*/?>
									<?php if (!empty($user_book_self_id)) {?>
									<ul class="tabs view_tab4" id="view_tab4">
										<li><a href="#" name="tab4<?php echo $details_r['share_id'];?>"><img alt="" src="<?=base_url()?>images/discevory_view.png" /></a></li>
									</ul>
									<?php } else {?>
									<img alt="" src="<?=base_url()?>images/discevory_view2.png" />
									<?php }
		?>
								...
								<td class="center">
                               <input id="check_<?php echo $details_r['share_id']?>" name="check_<?php echo $details_r['share_id']?>" type="checkbox" value="1" class="del_t" onclick="del_share_by(<?php echo $details_r['share_id']?>)"/>
                                </td>
							</tr>
							<?php $i++;}} else {?>
                        <tr>
								<td colspan="6">
									<p>No BookShelves are Shared.</p>
								</td>
							</tr>
                        <?php }
?>
                ...
                <?php if (!empty($share_bookshelf_test)) {
	$i = 1;
	foreach ($share_bookshelf_test as $details_test) {
		$workdetails_total = $this->mwork->allWorkForBookshelf($details_test['id']);
		//echo '<pre/>';print_r($workdetails_total);
		?>
				<div class="work_tab45">
					<div style="display: none;" id="tab4<?php echo $details_test['share_id'];?>" >
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_new orange1234" id="table_bookshelf">
							<thead>
								<tr>
									<th align="center" width="70%" colspan="3"><?php echo $details_test['name'];?></th>
...
?>
        ...
        <div class="work_main_tab3">
         <div class="work_tab54">
                ...
                <?php
//echo '<pre/>';print_r($share_bookshelf_with);die;
...
if ($share_bookshelf_with_count > 5) {
	?>
							<tr class="hov_no">
								<td colspan="6">
									<div class="button_right"><?php if ($offset == "0") {?>&nbsp;<?php } else {?> <a href="javascript:;" class="green_but" onclick="ajaxDiscovery_with(<?php echo $page - 1;?>)">PREVIOUS</a> <?php }
	?> <?php if ($share_bookshelf_with_count > $offset) {?><a href="javascript:;" onclick="ajaxDiscovery_with(<?php echo $page + 1;?>)" class="blue_but">VIEW MORE</a><?php } else {?><a href="javascript:;" class="blue_but">VIEW MORE</a> <?php }
	?></div>
								</td>
							</tr>
                           <?php }
	...
	foreach ($share_bookshelf_with as $details_test) {
		...
		?>
				<div class="work_tab55">
					<div style="display: none;" id="tab5<?php echo $details_test['share_id'];?>" >
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_new orange1234" id="table_bookshelf">
							<thead>
								<tr>
									<th align="center" width="70%" colspan="3"><?php echo $details_test['name'];?></th>
...
<?php /*<div class="content_share" id="myDiv">
<h2>Share the bookshelf "<span id="bkslf_name"></span>" with Addresses:</h2>
<?php
$frmAttrs   = array("id"=>'addBookshelf',"class"=>'form-horizontal',"name"=>'myform');
echo form_open('bookshelves/BookShelves_share', $frmAttrs);
?>
<label class="share_eml">Email:</label>
<!--<input id="tags_1" name="email[]" type="text" value="" class="tags"/> -->

<select data-placeholder="Please Select Email" class="chosen-select titinp_box3" multiple="multiple" name="email[]" id="email" tabindex="4">
<?php
$publisher = $this->mbookshelf->get_publishers();
if(isset($publisher) && count($publisher) > 0)
{
...
<option value="<?php echo $eachList['email']?>"><?php echo $eachList['name_first'].' '.$eachList['name_last'];?></option>
<?php } } ?>
...
<div class="clear"></div>
<!--<h2>Note:</h2>-->
<p class="bkslf_msg">A message will be sent to the following AEPs within inkubate who you are connected with</p>
<!--<textarea rows="3" cols="40" id="note" name="note"></textarea>-->
<div class="clear"></div>
<input type="hidden" id="bself_id" name="bself_id" value="" />
<input name="button" class="add_bkslf" type="submit" value="Send" style="margin-left: 46px !important;" />
</form>

</div> */?>


<div class="share_bookself" id="myDiv">
<p class="share_p">Share the bookshelf, <span id="bkslf_name"></span> with:</p>
...
<!--<textarea name="txt" cols="" rows="" class="textarea"></textarea>
  <select data-placeholder="Please Select Email" class="chosen-select titinp_box3" multiple="multiple" name="email[]" id="email" tabindex="4">

  </select>-->


    <select id="select-to" class="contacts" name="email[]" placeholder="Please Select Email...">
...
<p class="msg_p">A message will be sent to the following AEPs within Inkubate who you are connected with.</p>
...
function openDialog(id)
{
    ...
    $.ajax({
        ...
        success  : function(resp){
            ...
            //alert(resp.workdetails_test.pitch_count);
            ...
            if(resp.workdetails_test.pitch_count > 0)
            {
             ...
             $("#pitchit_img_icon").html("<img src=<?php echo base_url()?>"+"images/icon_p.png>");

            }
            else
            {
              $(".pitchit_pop_icon").attr("onClick", "FnComposeMail('"+resp.workdetails_test.user_id+"', '"+resp.workdetails_test.full_name+"')");
             $("#pitchit_img_icon").html("");
            }
            if(resp.workdetails_test.pitch_count_conversation > 0)
            {
              ...
              $("#conversation_img_icon").html("<img src=<?php echo base_url()?>"+"images/think.png>");
            }
            else
            {
              $("#conversationMail").attr("onClick", "FnComposeMail('"+resp.workdetails_test.user_id+"', '"+resp.workdetails_test.full_name+"')");
              $("#conversation_img_icon").html("");
            }
...
function openDialog_share(id)
{
    ...
    $.ajax({
        ...
        success  : function(resp){
            ...
            //alert(resp.workdetails_test.pitch_count_conversation);

            $(".pit_work_dialog_lat_first_share").css('display', "block");
            $("#title2").html(resp.workdetails_test.title_text)
            $("#excerpt2").html(resp.workdetails_test.extract)
            $("#synopsis2").html(resp.workdetails_test.synopsis)
            $("#review2").html(resp.workdetails_test.self_published_text);
            ...
            $(".pit_work_dialog_lat_first_share").dialog({
                close: function () {
	...
	$(document).ready(function () {
        ...
        $('#cancl_pit_share').click(function () {

         $(".pit_work_dialog_lat_first_share").dialog('close');
	   ...
	   $('.tabs a').click(function(e) {
	       ...
	       if ($(this).closest("li").attr("id") == "current"){ //detection for current tab
	        return;
	         ...
	         if( $('#' + $(this).attr('name')) == "tab1" &&  $('#' + $(this).attr('name')) == "tab3"  &&  $('#' + $(this).attr('name')) == "tab4" &&  $('#' + $(this).attr('name')) == "tab5"){
      ...
      $('.tabs_new a').click(function(e) {
	       ...
	       if ($(this).closest("li").attr("id") == "current"){ //detection for current tab
	        return;
	         ...
	         $("#content").find("[id^='tab']").hide(); // Hide all content
	         $(".tabs_new li").attr("id",""); //Reset id's
	         if( $('#' + $(this).attr('name')) == "tab1" &&  $('#' + $(this).attr('name')) == "tab3"  &&  $('#' + $(this).attr('name')) == "tab4" &&  $('#' + $(this).attr('name')) == "tab5"){
	         	$(this).parent().attr("id","current"); // Activate this
	         	$('#' + $(this).attr('name')).fadeIn(); // Show content for the current tab
	         }
...
<script type="text/javascript">
    ...
    var config = {
      //f'.chosen-select'           : {max_selected_options: 3},
