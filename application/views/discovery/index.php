...
<script type="text/javascript" src="<?=base_url()?>js/hilitor.js"></script>
...
.highlightWord { background-color: #72E774; }
</style>
<script>
format = 0;
types = 0;
...
function selectMultiple(type)
{
	...
	if(type == "format")
	{
        ...
        if($("#dropdown_two").css("display") == 'none'){
            $("#replace_down_two").attr('src', base_url+'images/arrow_down.png');
        }else{
	...
	if(type == "genre")
	{
        ...
        //$("#dropdown_five").hide();
        $("#dropdown_six").hide();
        $("#dropdown_five").toggle();
        if($("#dropdown_five").css("display") == 'none'){
            $("#replace_down_five").attr('src', base_url+'images/arrow_down.png');
        }else{
	...
	if(type == "multiple")
	{
        ...
        //$("#dropdown_six").hide();
        $("#dropdown_six").toggle();
        if($("#dropdown_six").css("display") == 'none'){
            $("#replace_down_six").attr('src', base_url+'images/arrow_down.png');
        }else{
	...
	if(type == "types")
	{
        ...
        $("#dropdown_five").hide();
        $("#dropdown_six").hide();
        $("#dropdown_four").toggle();
        if($("#dropdown_four").css("display") == 'none'){
            $("#replace_down_four").attr('src', base_url+'images/arrow_down.png');
        }else{
...
function ajaxDiscovery_global_search(page,str,stat)
{
    //console.log('hi');
	...
	if(format == 1)
 	{
 		...
 		$.ajax({
		   url      : '<?=base_url()?>'+'discovery/ajax_discovery_global_search',
		   type     : 'POST',
		   data     : { 'page': page, 'str': str, 'stat':stat, format:format_str, type:type},
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            $("#tab1").html(resp);
 	...
 	else if(types == 1)
 	{
 		...
 		$.ajax({
		   url      : '<?=base_url()?>'+'discovery/ajax_discovery_global_search',
		   type     : 'POST',
		   data     : { 'page': page,  'str': str, 'stat':stat, types:types_str,type:type },
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            $("#tab1").html(resp);
 	...
 	else if(genre == 1)
 	{
 		...
 		$.ajax({
		   url      : '<?=base_url()?>'+'discovery/ajax_discovery_global_search',
		   type     : 'POST',
		   data     : { 'page': page,  'str': str, 'stat':stat, genre:genre_str,type:type },
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            $("#tab1").html(resp);
 	...
 	else if(multiple == 1)
 	{
        ...
        //alert(types_str);
 		...
 		$.ajax({
		   url      : '<?=base_url()?>'+'discovery/ajax_discovery_global_search',
		   type     : 'POST',
		   data     : { 'page': page,  'str': str, 'stat':stat,  genre:genre_str, types:types_str, format:format_str, type:type, subtype:subtype, searchCriteria:searchCriteria },
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            $("#tab1").html(resp);
 	...
 	else
 	{
 		...
 		$.ajax({
		   url      : '<?=base_url()?>'+'discovery/ajax_discovery_global_search',
		   type     : 'POST',
		   data     : { 'page': page , 'str': str, 'stat':stat},
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            $("#tab1").html(resp);
...
function openDialog(id)
 {
 	...
 	$.ajax({
       ...
       success  : function(resp){
           ...
           var full_name = resp.workdetails_test.name_first+' '+resp.workdetails_test.name_middle+' '+resp.workdetails_test.name_last;
           //$(".pitchit_pop_icon").attr("onClick", "FnComposeMail('"+resp.workdetails_test.user_id+"', '"+resp.workdetails_test.full_name+"')");
           $("#think_icn").attr("onClick", "FnComposeMail('"+resp.workdetails_test.user_id+"', '"+full_name+"')");
           ...
           if(resp.workdetails_test.work_file != '')
           {
            ...
            $("#download").html('<a href="<?=base_url()?>discovery/download/'+id+'/'+resp.workdetails_test.file_asset_id+'/'+resp.workdetails_test.user_id+'/'+resp.workdetails_test.work_file+'">Download Title</a>');
...
function openDialog_global_serch(id,str,stat)
 {
    //var keyword = $('#keyword_'+id).attr('data-fruit');
 	//alert($('#keyword_'+id).attr('data-fruit'));
    //var strng = str.split(' OR ');
    //var new_string = strng.join(' ');
    //alert(str);
    //alert(stat);
    if(stat == 2)
    {
     ...
     $("#pit_work_dialog").css('display', "block");
           ...
           $("#title_saved").css('display','none');
       	   ...
       	   $(".pit_work_dialog_vw").dialog();
                ...
                {
                    //alert(stat+'prasen');
                $("#pit_work_dialog *").highlight(str, "highlightWord");
                ...
                { 
                    //alert('hi');
                var myHilitor2;
                myHilitor2 = new Hilitor("pit_work_dialog");
                myHilitor2.setMatchType("left");
                myHilitor2.apply(str);
                }
 ...
 function fnSearchByFilters(type)
 {
 	if(type == "format" && format == 1)
 	{
        ...
        $('.li_pad2').css('background','#46b8ff');
        $('.li_pad2').css('color','#fff');
		   ...
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            ...
		            $("#savedSearchBut").html('<a href="javascript:void(0);" onclick="openSaveBox(\'format\')"><img alt="" src="<?=base_url();?>images/circle_edit.png"></a>');
		            //$("#edit_class" ).dialog( "close" );
		        }
		        $("#content_part").unblock();
		   },
 	...
 	if(type == "types"  && types == 1)
 	{
        ...
        $('.li_pad1').css('background','#46b8ff');
        $('.li_pad1').css('color','#fff');
 		...
 		$.ajax({
		   ...
		   success  : function(resp){
		        ...
		        if(resp != '0'){
		            $("#tab1").html(resp);
		            $("#show_search").html( $("#show_search_inp").val());
		            $("#savedSearchBut").html('<a href="javascript:void(0);" onclick="openSaveBox(\'types\')"><img alt="" src="<?=base_url();?>images/circle_edit.png"></a>');
		            $("#showSearchOptions").html($("#searchOptionsId").html());
		            //$("#edit_class" ).dialog( "close" );
		        }
		        $("#content_part").unblock();
 	...
 	if(type == "genre" && genre == 1)
 	{
        ...
        $('.li_pad3').css('background','#46b8ff');
        $('.li_pad3').css('color','#fff');
		   ...
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            $("#tab1").html(resp);
		            $("#show_search").html( $("#show_search_inp").val());
		            $("#savedSearchBut").html('<a href="javascript:void(0);" onclick="openSaveBox(\'genre\')"><img alt="" src="<?=base_url();?>images/circle_edit.png"></a>');
		            $("#showSearchOptions").html($("#searchOptionsId").html());
		            //$("#edit_class" ).dialog( "close" );
		        }
		         $("#content_part").unblock();
 	...
 	if(type == "multiple" && multiple == 1)
 	{
        ...
        $('.li_pad4').css('background','#46b8ff');
        $('.li_pad4').css('color','#fff');
		   ...
		   success  : function(resp){
		    //alert(resp);
		        if(resp != '0'){
		            ...
		            $("#show_search").html( $("#show_search_inp").val());
		            $("#showSearchOptions").html($("#searchOptionsId").html());
		            $("#savedSearchBut").html('<a href="javascript:void(0);" onclick="openSaveBox(\'multiple\')"><img alt="" src="<?=base_url();?>images/circle_edit.png"></a>');
		            //$("#edit_class" ).dialog( "close" );
		        }
		        $("#content_part").unblock();
		   },
 ...
 function savedSearchesCount()
	...
	$("#replace_down_six").attr('src', base_url+'images/arrow_down.png');
      ...
      $('.li_pad1').css('background',''); 
      $('.li_pad1').css('color','');
      
      $('.li_pad2').css('background',''); 
      $('.li_pad2').css('color',''); 
      //alert('hi'); 
      $('.li_pad3').css('background',''); 
      $('.li_pad3').css('color','');
      
      $('.li_pad4').css('background',''); 
      $('.li_pad4').css('color','');
 ...
 function fnDeleteSearches(id,checkbox)
 {
 	if(checkbox.checked == true)
 	{
	 	...
	 	if(confirm('Are you sure to delete this saved search item?'))
		{
			$("#content_part").block();
	 		$.ajax({
				...
				if(resp != '0'){
				   ...
				   if($("#type").val() == "multiple")
				   {
				   	...
				   	if($("#multiple_genre").val() != "")
				   	{
				   	 ...
				   	 $("#savedSearchBut").html('<a href="javascript:void(0);" onclick="openSaveBox(\'global\')"><img alt="" src="<?=base_url();?>images/circle_edit.png"></a>');
				   	//$("#showSearchOptions").html($("#searchOptionsId").html());
			 		$("#showSearchOptions").html('');
				   }
...
<!--<li ><a href="javascript:void(0);" onclick="openSaveBox('types')" class="orange_bg" name="tab1">Click to Save for Multiple Filters</a></li>-->
</ul>
</div>

</li>
<li id="current" class="li_pad li_pad2">
...
<?php } ?>
<li><a href="javascript:void(0);" class="green_bg" name="tab1" onclick="fnSearchByFilters('format')">Click to Search by Filters</a></li>
<!--<li><a href="javascript:void(0);"  class="orange_bg" name="tab1" onclick="openSaveBox('format')">Click to Save for Multiple Filters</a></li>-->
</ul>
</div>
</li>
<li id="current" class="li_pad li_pad3">
...
if($count == $catcount -1)
{
	...
	//echo '<li><a href="javascript:void(0);" class="orange_bg" onclick="openSaveBox(\'genre\')" name="tab1">Click to Save for Multiple Filters</a></li>';
}
...
<li id="current" class="li_pad li_pad4">
     <span id="replace_plus_six" class="li_span" onclick="selectMultiple('multiple')"><img src="<?=base_url()?>images/filter_icon04.png" style="float: left; margin-right: 10px;" alt="" />Advanced Search <img src="<?=base_url()?>images/arrow_down.png" alt="" class="right_flo"  style="cursor:pointer; margin-top: 9px" id="replace_down_six" /></span>
...
if($count == $catcount -1)
{
	echo '<li><a href="javascript:void(0);" class="green_bg" name="tab1"  onclick="fnSearchByFilters(\'multiple\')">Click to Search by Filters</a></li>';
}
...
<li><a href="javascript:void(0);" name="tab5" onclick="savedSearchesCount()"><img src="<?=base_url()?>images/circle_edit.png" alt="" /> Saved Searches</a></li>
            </ul>
          ...
          <div class="pitchits_section_right" id="content">
			...
			<?php echo substr(ucwords($details['name_first'].' '.$details['name_middle'].' '.$details['name_last']),0,10)."..."?>
 ...
 <div class="clear"></div> -->
             ...
             <?php echo ucwords($details['name_first'].' '.$details['name_middle'].' '.$details['name_last']);?>
                    </a>
             <?php } ?> 
                ...
                <td align="center">
                 <a class="tooltips" onclick="openDialog('<?php echo $details['id'];?>')">
                     ...
                     <label style="cursor: pointer;" title="<?php echo ucwords($details['title']);?>"><?php echo substr(ucwords($details['title']),0,15)."...";?></label> 
                     <!-- <span class="tp_span_disc"><?php //echo $details['title'];?></span>
                     <div class="clear"></div> -->

                 <?php } else {?>
                     <?php echo ucwords($details['title']);?>
                     <?php } ?> 
                 </a>
                </td>
...
<!--                  <td align="center" ><span style="cursor: pointer;"  onclick="openDialog(<?php echo $details['id'];?>)"><?php if(strlen($details['title']) > 15){ echo substr($details['title'],0,15)."..."; }else{ echo $details['title']; }?></span></td>-->
           ...
           <div class="button_right"><?php if($offset == "0"){?>&nbsp;<?php }else{ ?> <a href="javascript:void(0);" class="green_but" onclick="ajaxDiscovery(<?php echo $page-1;?>)">PREVIOUS</a>
            <?php } ?> <?php if($total_rows > $offset){ ?><a href="javascript:void(0);" onclick="ajaxDiscovery(<?php echo $page+1;?>)" class="blue_but">VIEW MORE</a>
            <?php }else{?> ><a href="javascript:void(0);" class="blue_but">VIEW MORE</a> <?php } ?></div>
...
<span id="download"></span> 
                    <a  href="#bookshelf_tt" id="ab" rel="facebox" style="text-align: center; width:auto;" class="ab">Add to Bookshelf</a>
 ...
 <!-------------------------End Popup--------------------------------------------->     
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_new">
              ...
              <tbody>
                ...
                <tr id="tr<?php echo $val['id'];?>">
                  <td align="center">
                  	<?php if(strlen($val['saved_search_name']) > 25) {?>
                  	<div title="<?php echo $val['saved_search_name'];?>"><?php echo substr($val['saved_search_name'],0,22)."...";?></div>
					<?php } else {?>
                    <?php echo $val['saved_search_name'];?>
                    <?php } ?> 
                  </td>
           ...
           <?php if($total_rows_ss > 6) { ?> 
            
           <div class="button_right">
           <?php if($offset == "0"){?>&nbsp;<?php }else{ ?> <a href="javascript:void(0);" class="green_but" onclick="ajaxSavedSearches(<?php echo $page_ss-1;?>)">PREVIOUS</a> <?php } ?> <?php if($total_rows_ss > $offset){ ?><a href="javascript:void(0);" onclick="ajaxSavedSearches(<?php echo $page_ss+1;?>)" class="blue_but">VIEW MORE</a><?php }else{?> <a href="javascript:void(0);" class="blue_but">VIEW MORE</a> <?php } ?>
           </div>
                ...
                success:function(data){
                    ...
                    if(data.status == "1")
                    {
                    	...
                    	for (var i = 0, p; p = ps[i++];) 
                        {
                            ...
                            html += '<li>';
                            html += '<a href="javascript:void(0);" onclick="FnAddEmail(\''+p.id+'\',\''+name+'\')">'+name+'</a>' ;
...
function FnComposeMail(id,name)
{
	...
	$.ajax({
        type:'POST',
        url:BASE+'discovery/getMessageIdByWid',
		data:{wid:wid},
        success:function(message_id){
            if(message_id != "0")
            {
            	//alert(message_id);	
            	window.location = '<?=base_url()?>mail/details/'+message_id;
            }else{
            	alert("There are no conversation against this work");
            }
        }
 	});
