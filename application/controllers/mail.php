...
var $author_per_page = 10;
  ...
  public function compose(){   
   ...
   public function details()
	{
       ...
       $this->load->view('mail_detail',$data);*/
        ...
        {
            //$this->memail->update_view($this->uri->segment(3),$this->uri->segment(4));
            $this->memail->update_view($this->uri->segment(3));
            $data['user_contact'] = $this->mprofile->get_user_iscontact();
            
            
              $data['single_pitch_details']  = $this->memail->singlePitchInfo($this->uri->segment(3));
            ...
            $data['from_user_pitch']  = $this->memail->fromUser_pitch($this->uri->segment(3));
            
            $data['to_user']  = $this->memail->toUser();
   ...
   public function delete_details(){
	...
	$getAllChilds = $this->memail->getAllChildsMails($mainid);
	
	//Delete Main email if all subs are deleted
	if($getAllChilds ==count($arr))
	{
	    //Get Main email, deleted from list or not
	     $delete_main = $this->memail->msgMainMail($mainid);
	     if($delete_main !=0)
	     {
		$this->memail->msgDelete($mainid);
	     }
	}
	//echo $getAllChilds;die;
	if($getAllChilds)
	{
	    if($getAllChilds==1)
	    {
	...
	else
	{
        ...
        $getAllChildsAfterDelete = $this->memail->getAllChildsMails($mainid);
	
	
	$getparentmail = $this->memail->getParentMails($mainid);
	
        if($this->uri->segment(5) == 'ALL'){       
            redirect('home/inbox','refresh');       
        } else {
	    if($getAllChildsAfterDelete==0 AND $getparentmail==0)
	    {
		 redirect('home/inbox','refresh'); 
	    }
	    else
    ...
    public function msgDeleteFromList()
    {
	
	$val = $this->uri->segment(4);
	$this->memail->msgDeleteFromList($val);
	redirect('mail/details/'.$this->uri->segment(3), 'refresh');
    }
