...
class Home extends CI_Controller {
	...
	public function forget_pass_email_exists() {
		//if($this->session->userdata('logged_user')){
		if ($this->input->post(null)) {
			echo $this->mhome->forget_pass_email_exists();
		}
		//}
       ...
       $data['uri3'] = $this->uri->segment(3);
		...
		if ($this->session->userdata('logged_user')) {
			...
			if ($this->input->post(null)) {
				...
				$bookshelf_url = $this->input->post('bookshelves');
                $msg_url = $this->input->post('msg');

				//echo $bookshelf_url;die;
				...
				if ($valid == '1') {
					...
					} elseif (!empty($bookshelf_url)) {
						redirect(base_url() . $bookshelf_url, 'refresh');
					} 
                    elseif (!empty($msg_url)) {
						redirect(base_url() . 'home/inbox', 'refresh');
					} 
                    else {
						redirect('home/author', 'refresh');
					}
	...
	public function author() {
		...
		if ($this->session->userdata('logged_user')) {
			...
			if ($usd['user_type'] == '1') {
				...
				$data['a_male_percent'] = $this->memail->get_a_male_percent();
                $data['e_male_percent'] = $this->memail->get_e_male_percent();
                $data['p_male_percent'] = $this->memail->get_p_male_percent();
                $data['aep_male_percent'] = $data['a_male_percent'] + $data['e_male_percent'] + $data['p_male_percent'];
                
				$data['a_female_percent'] = $this->memail->get_a_female_percent();
                $data['e_female_percent'] = $this->memail->get_e_female_percent();
                $data['p_female_percent'] = $this->memail->get_p_female_percent();
                $data['aep_female_percent'] = $data['a_female_percent'] + $data['e_female_percent'] + $data['p_female_percent'];
                
				$data['total_a'] = $this->memail->get_a_total();
                $data['total_e'] = $this->memail->get_e_total();
                $data['total_p'] = $this->memail->get_p_total();
                $data['total_aep'] = $data['total_a'] + $data['total_e'] + $data['total_p'];
				...
				if (!empty($user_contact['name_last'])) {$val = $val + 1;}
				if (!empty($user_contact['email'])) {$val = $val + 1;}
	...
	public function publisher() {
		if ($this->session->userdata('logged_user')) {
			...
			if ($usd['user_type'] == '2' || $usd['user_type'] == '3' || $usd['user_type'] == '4') {
				...
				/*$data['pitchit_details_limit2'] = $this->memail->get_main_pitchit();
				if(!empty($data['pitchit_details_limit2']) && !empty($data['pitchit_details_limit1']) )
				{
				$data['pitchit_details_limit'] = array_merge($data['pitchit_details_limit2'],$data['pitchit_details_limit1']);
				}
				else
				{
				$data['pitchit_details_limit'] = $data['pitchit_details_limit1'];
				} */
	...
	public function doPitchit() {
		...
		if ($this->session->userdata('logged_user')) {
			...
			echo $data['pit_cnt']['count'] . '~' . $this->load->view('ajax_search/ajax_latest_pitchit_search_author_org', $data);
	...
	public function pitchits_inbox() {
		if ($this->session->userdata('logged_user')) {
			...
			/*$data['mail_details2'] = $this->memail->get_main_org_pitchit();

			if(!empty($data['mail_details1']) || !empty($data['mail_details2']))
			{
			$data['mail_details'] = array_merge($data['mail_details2'],$data['mail_details1']);
			} */
			...
			/*$data['pitchit_count2']  = $this->memail->work_pitchitCount();
			$data['pitchit_count']  = $data['pitchit_count2'] + $data['pitchit_count1']; */
	...
	public function ajax_package_price() {
		if ($this->session->userdata('logged_user')) {
			...
			$total_pitch_allow = $this->memail->get_purchase_pitchit_total();
			$total_pitchited = $this->memail->get_use_pitchit_count();

			$total_allow = $total_pitch_allow['sum_total'] - $total_pitchited['count'];
	...
	public function fblogin() {
		...
		if ($user) {
			...
			$data['login_url'] = $this->facebook->getLoginUrl(array(
				'redirect_uri' => site_url('home/fblogin'),
				'scope' => array("email"), // permissions here
			));
		}
		$this->load->view('fbusertype', $data);
	...
	public function user() {
		...
		if ($this->input->post(null)) {
			if ($this->input->post('writer') == '1') {
				redirect('home/author', 'refresh');
			}
			if ($this->input->post('publisher') == '2') {
				redirect('home/publisher', 'refresh');
			}
	...
	public function forget_password() {
		...
		if ($this->input->post(null)) {
			//echo '<pre>';print_r($this->input->post());
			$this->mhome->forget_password();

			$data['page'] = 'Forget Confirmation';
			$data['title'] = 'The Inkubate - Forget Password';
			$this->load->view('forget_confirmation', $data);

		} else {
			//echo "here";die;
			$data['page'] = 'Forget Password';
			$data['title'] = 'Inkubate Forget Password';
	...
	public function success_sign() {
		...
		$this->mhome->success_sign($this->uri->segment(3));
