...
class Bookshelves extends CI_Controller {
  ...
  public function index($search = ""){
    if($this->session->userdata('logged_user')){
        ...
        /*this portion is for showing authorlist*/
            ...
            if($usd['user_type'] == '2' || $usd['user_type'] == '3')
            {
                ...
                $data['share_bookshelf_test'] = $this->mbookshelf->get_bookshelf_by_share($search,$offset,$limit);
                $data['share_bookshelf_test_count'] = $this->mbookshelf->get_bookshelf_by_share_count($search,$offset,$limit);
                
                $data['share_bookshelf_with_total'] = $this->mbookshelf->get_bookshelf_with_share_total($search,$offset,$limit);
                $data['share_bookshelf_with'] = $this->mbookshelf->get_bookshelf_with_share($search,$offset,$limit);
                $data['share_bookshelf_with_count'] = $this->mbookshelf->get_bookshelf_with_share_count($search,$offset,$limit);
            ...
            //redirect(base_url().'home/login/bookshelves', 'refresh');
            $data33['page']   = 'login';
            $data33['title']  = 'The Inkubate - Login';
            $data33['login_url'] = $this->facebook->getLoginUrl(array(
                'redirect_uri' => site_url('fblogin/index'), 
                'scope' => array("email") // permissions here
            ));
            $data33['bookshelves'] = 'bookshelves';
            $this->load->view('login', $data33);
        }
    } 
    ...
    public function ajax_bokkshelf_with($search = ""){
        if($this->session->userdata('logged_user')){
        ...
        //$data['total_rows']  = $this->mwork->getCountAuthorRecentTitle();
        $data['total_rows']  = $this->mbookshelf->get_bookshelf_with_share_count($search);
        $data['share_bookshelf_with'] = $this->mbookshelf->get_bookshelf_with_share($search,$data['offset'],$limit);
        $data['share_bookshelf_with_count'] = $this->mbookshelf->get_bookshelf_with_share_count($search);
    		
    		$this->load->view('bookshelf/ajax_bokkshelf_with',$data);
        }
 ...
 public function BookShelves_share(){
  ...
  public function delete_bookshelf_new()
   ...
   public function delete_bookshelf_share_with()
	{
	   $data['title']  = 'My Bookshelves';
       //echo $this->uri->segment(3);die;
        ...
        if($this->input->post(null)){
                ...
                //print_r($this->input->post('id'));die;
             $this->mbookshelf->delete_bookshelf_share_with($this->input->post('id')); 
      
	} 
  }
  
  public function delete_bookshelf_share_by()
	{
	   $data['title']  = 'My Bookshelves';
       //echo $this->uri->segment(3);die;
        ...
        if($this->input->post(null)){
                ...
                //print_r($this->input->post('id'));die;
             $this->mbookshelf->delete_bookshelf_share_by($this->input->post('id')); 
 ...
 public function booklist(){
  ...
  function show_book_detail()
  {
     ...
     $fullname = $data['workdetails_test']['name_first'].' '.$data['workdetails_test']['name_middle'].' '.$data['workdetails_test']['name_last'];
    //$data['workdetails_test']['message_id'] = $this->mwork->getMessageIdByWid( $wid );
  	...
  	$data['workdetails_test']['title_text'] = $data['workdetails_test']['title']." by ".$fullname." was added on ".$data['workdetails_test']['create_date'];
