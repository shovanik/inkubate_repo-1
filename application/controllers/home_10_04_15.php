...
class Home extends CI_Controller {
  ...
  public function user_details_search()
  {
	   ...
	   if($this->session->userdata('logged_user')){
          ...
          if($usd['user_type'] == '2'){
           ...
           //Latest Pitchit Portion Start//
            ...
            $data['offset_latest']  = $offset_latest;
            $data['page_latest']    = $page_latest;
            $data['userLatestPitchitDetails']  = $this->mpitchit->getUserLatestPitchitDetails($offset_latest,$limit_latest);
            ...
            $data['offset_saved']   = $offset_saved;
            $data['page_saved']     = $page_saved;
            //$data['userSavedPitchitDetails']    = $this->memail->get_user_pitchit_details_publisher($offset_saved,$limit_saved);
            $data['userSavedPitchitDetails']    = $this->mpitchit->getUserSavedPitchitDetails($offset_saved,$limit_saved);
             $data['userSavedPitchitCount']    = $this->mpitchit->getUserSavedPitchitCount();
            ...
            $limit_viewall    = 5;
            $page_viewall     = 1;
            $offset_viewall   = ($page_viewall - 1) * $limit_viewall;
            $data['offset_viewall']   = $offset_viewall;
            $data['page_viewall']     = $page_viewall;
            $data['userViewallPitchitDetails']    = $this->mpitchit->getUserAllPitchitDetails($offset_viewall,$limit_viewall);
            ...
            $limit_resp    = 5;
            $page_resp     = 1;
            $offset_resp   = ($page_resp - 1) * $limit_resp;
            $data['offset_resp']   = $offset_resp;
            $data['page_resp']     = $page_resp;
            ...
            $data['pitchit_details_limit_original'] = array_slice ($data['pitchit_details_limit_total'], $offset_resp, $limit_resp);
            //$data['total_response_cnt']  = $this->memail->get_user_pitchit_msg_total_cnt();
            //print_r($data['pitchit_details_limit_total']);die;
            //pitchits responses portion End//

            //Total Viewed pitchits portion Start//
            $limit_total    = 5;
            $page_total     = 1;
            $offset_total   = ($page_total - 1) * $limit_total;
            $data['offset_total']   = $offset_total;
            $data['page_total']     = $page_total;
            //$data['totalViewPitchit']  = $this->memail->get_publisher_totalview_pitchit();
            $data['totalViewPitchit']    = $this->mpitchit->getTotalViewPitchit($offset_total,$limit_total);
                        ...
                        foreach($json->channel->item as $row){
                            ...
                            if ($media->content && $media->content->attributes()) {
                                ...
                                if($image == "PublishersWeekly.com"){
                                    ...
                                    $feeds_array['image'] = '<img src="'.base_url().'images/news_icon_das.png" alt="" />';
                                }
   ...
   public function ajax_author_rcenttitle_search(){
    ...
    public function ajax_saved_pitchit_search(){
        if($this->session->userdata('logged_user')){
            ...
            $data['user_pitchit_details']  = $this->memail->get_user_pitchit_details_publisher($data['offset_saved'],$limit_saved);
            $data['total_pit_save_cnt']  = $this->memail->pitchit_save_cnt();
            $data['view_more']  = (count($data['total_pit_save_cnt'])-($limit_saved*$data['page_saved']));
